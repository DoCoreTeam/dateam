...
- 월(month/月/월/mo): ÷ 720    · 주(week): ÷ 168    · 일(day/日): ÷ 24    · 년(year/年): ÷ 8760
  예) $138.54/월 → price_usd: 0.19, notes: "원본: $138.54/월 (÷720 시간환산)"
- 여러 장(예: 8GPU 묶음 가격)이면 장수로 나눠 1장당으로. notes에 원본 기재.

【중요 — 통화는 "있는 그대로" 보존 (임의 환율 변환 절대 금지)】
환율로 USD 변환하지 마세요. 입력에 보이는 통화·금액을 그대로 보존만 하세요. USD 환산은 서버가 실환율(fx_rates)로 처리합니다.
- original_currency: 입력 통화의 ISO 코드("KRW"|"USD"|"JPY"|"EUR" 등). 원/₩/P(포인트)/C(크레딧)=KRW, $=USD.
- original_price: 그 통화 기준 "GPU 1장·1시간당" 금액(시간/장수 환산만 적용 — 통화 변환은 하지 말 것).
- price_usd: 입력이 이미 USD($)면 그 값, USD가 아니면 반드시 null(서버가 환산). 임의 환율로 채우지 마세요.
- 시간/장수 정규화(÷720 등)는 위 규칙대로 적용하되 "통화 변환"만 하지 않는 것입니다.
    ...
    if (classifyRes.ok) {
      ...
      try {
        ...
        if (classified.type === 'competitor' && Array.isArray(classified.items) && classified.items.length > 0) {
          // 원본 통화 보존(W1) — AI는 통화 변환 안 함(price_usd=null). 비USD는 서버가 fx_rates 실환율로 환산.
          //   원본(original_currency·original_price)은 그대로 보존 → 저장·표시에 활용.
          const items = classified.items as CompetitorPriceItem[]
          const needsFx = items.some((it) => it && it.original_currency && it.original_currency !== 'USD' && it.price_usd == null)
          if (needsFx) {
            const admin = createAdminClient()
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const { data: fxRow } = await (admin.from('fx_rates') as any).select('usd_krw').order('rate_date', { ascending: false }).limit(1).maybeSingle()
            const krwPerUsd = typeof fxRow?.usd_krw === 'number' && fxRow.usd_krw > 0 ? fxRow.usd_krw : 1400
            for (const it of items) {
              if (!it || it.price_usd != null) continue
              if (it.original_currency === 'KRW' && typeof it.original_price === 'number' && it.original_price > 0) {
                try { it.price_usd = toUsdPerGpuHour({ amount: it.original_price, currency: 'KRW', period: 'hour', gpuCount: 1, krwPerUsd }) } catch { /* 환산 불가 — null 유지(가격 미상) */ }
              }
            }
          }
          // ── 경쟁사 가격: 자동 저장하지 않고 "미리보기"만 반환 → 사용자가 '반영' 눌러야 저장
          const preview = dedupCompetitor(items as CompetitorLike[])
