...
export async function POST(req: NextRequest) {
  ...
  if (body.model_name) {
    ...
    const { data: products } = await (supabase as any).from('gpu_products').select('model_name').is('deleted_at', null) // eslint-disable-line @typescript-eslint/no-explicit-any
