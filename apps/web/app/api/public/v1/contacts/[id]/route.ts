...
export async function GET(request: NextRequest, { params }: Ctx) {
  ...
  try {
    ...
    return serverError('contacts/:id GET', err, { ctx: auth.ctx, request })
  }
}
...
export async function PATCH(request: NextRequest, { params }: Ctx) {
  ...
  try {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const admin = createAdminClient() as any
    // RLS 의 contacts_update_own·contacts_delete_own 과 같은 판정 — 서비스 롤이라 앱이 다시 본다
    const denied = await guardOwnedRow(admin, 'contacts', id, auth.ctx, request)
    ...
    return serverError('contacts/:id PATCH', err, { ctx: auth.ctx, request })
  }
}
...
export async function DELETE(request: NextRequest, { params }: Ctx) {
  ...
  try {
    ...
    return serverError('contacts/:id DELETE', err, { ctx: auth.ctx, request })
  }
}
