...
export async function PATCH(req: NextRequest, { params }: Ctx) {
  return withCrmApi('ADMIN', async ({ session }) => {
    ...
    return updateBusinessType(session.workspaceId, session.memberId, (await params).id, body)
  })
}

export async function DELETE(_req: NextRequest, { params }: Ctx) {
  return withCrmApi('ADMIN', async ({ session }) => {
    await deleteBusinessType(session.workspaceId, session.memberId, (await params).id)
    return { ok: true }
  })
}
