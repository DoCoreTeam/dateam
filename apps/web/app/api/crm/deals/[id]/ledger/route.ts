...
export async function GET(_req: NextRequest, { params }: Ctx) {
  return withCrmApi('READONLY', async ({ db, session }) =>
    toLedgerJson(await getLedger(db, (await params).id), await viewerOf(db, session)))
}
...
export async function PATCH(req: NextRequest, { params }: Ctx) {
  return withCrmApi('MEMBER', async ({ db, session }) => {
    ...
    return toLedgerJson(await updateLedgerMeta(db, (await params).id, body, session.memberId), viewer)
  })
}
