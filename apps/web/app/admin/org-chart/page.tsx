...
export default async function OrgChartAdminPage() {
  ...
  const [companyRes, nodesRes, profilesRes, ranksRes, positionsRes] = await Promise.all([
    ...
    db.from('profiles').select('id, name, rank, position, email').is('deleted_at', null).order('name'),
  ...
  const allProfiles = (profilesRes.data ?? []) as { id: string; name: string; rank: string | null; position: string | null; email: string | null }[]
