...
export default async function AdminReportsPage({ searchParams }: PageProps) {
  ...
  // 첫 진입(부서 미선택) 탐색성 — 전 부서 취합 상태 + 회사 전체 롤업(SSOT computeDeptAggStats).
  let deptAggStats: DeptAggStat[] = []
  let companyRollup: CompanyRollup | null = null
  if (!dept && !member) {
    const scope = await resolveOrgScope(adminClient, user.id)
    const agg = await computeDeptAggStats(adminClient, scope, deptNodes ?? [], selectedWeek)
    deptAggStats = agg.stats
    companyRollup = agg.rollup
  }
  ...
  return (
    <div>
      ...
      {!dept && !member && deptAggStats.length > 0 && (
        <DeptAggGrid stats={deptAggStats} rollup={companyRollup} weekStart={selectedWeek} />
      )}
