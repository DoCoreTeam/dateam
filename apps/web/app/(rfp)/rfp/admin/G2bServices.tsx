    ...
    <section className="card">
      <div className={styles.sectionHead}>
        <div className={styles.between}>
          <span className={styles.sectionTitle}>{RFP_ADMIN.g2bServices}</span>
          <NbBadge status="note">{inUse} / {G2B_SERVICES.length}</NbBadge>
        </div>
        <p className={styles.sectionDesc}>{RFP_ADMIN.g2bApplyHint}</p>
      </div>
      ...
      <div className={styles.ruleList}>
        {G2B_SERVICES.map((s) => (
          <div key={s.id} className={styles.ruleItem}>
            ...
            <NbBadge status={s.inUse ? 'done' : 'note'}>
