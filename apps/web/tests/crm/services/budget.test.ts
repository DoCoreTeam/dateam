...
async function reset(limit: bigint, spent: bigint = BigInt(0)) {
  ...
  await dbA.crmAiBudget.create({
    data: { workspaceId: WS_A, month: MONTH, limitMinorUsd: limit, spentMinorUsd: spent },
  })
}

async function cleanup() {
  await dbA.crmAiBudget.deleteMany({ where: { month: MONTH } })
  await dbA.crmAuditLog.deleteMany({ where: { action: 'budget.limit_changed' } })
}
...
test('예산 행이 없으면 기본 상한으로 만들어 준다 — 설정 안 했다고 AI 가 무제한이면 안 된다', async () => {
  await dbA.crmAiBudget.deleteMany({ where: { month: MONTH } })
  const r = await reserveBudget(WS_A, BigInt(0))
...
test('★ DI-15 한도에 닿으면 AI 호출이 막히고, 사용자가 무엇을 할지 알려 준다', async () => {
  ...
  const e = await catchError(() => reserveBudget(WS_A, BigInt(10)))
...
test('★ DI-15 차단돼도 코어 CRM 은 계속 돌아간다 — 영업이 멈추면 더 큰 손해다', async () => {
  ...
  const co = await createCompany(WS_A, 'mb_owner', { name: '예산차단중 회사' })
  assert.ok(co.id, '예산이 떨어졌다고 회사를 못 만들면 안 된다')

  await deleteCompany(WS_A, 'mb_owner', co.id, 'purge')
  await dbA.crmAuditLog.deleteMany({ where: { targetId: co.id } })
  await cleanup()
})

test('차단 상태가 기록에 남는다 — 언제부터 막혔는지 알 수 있어야 한다', async () => {
  await reset(BigInt(100), BigInt(100))
  await catchError(() => reserveBudget(WS_A, BigInt(1)))
  const b = await dbA.crmAiBudget.findFirst({ where: { month: MONTH } })
...
test('★ 선점하면 한도를 넘는 호출은 아예 보내지 않는다 — "넘으면 다음부터"는 사후 통보다', async () => {
  ...
  const e = await catchError(() => reserveBudget(WS_A, BigInt(50))) // 예상 50 → 140 이 된다
  assert.ok(e instanceof CrmError)
  assert.equal((e as CrmError).code, 'BUDGET_BLOCKED')

  const after = await dbA.crmAiBudget.findFirst({ where: { month: MONTH } })
...
test('잔여 안에 들어오는 호출은 그대로 통과한다 (과차단 방지)', async () => {
  ...
  const r = await reserveBudget(WS_A, BigInt(9))
...
test('80% 에 닿으면 경보 시각이 찍히고, 두 번 찍히지 않는다', async () => {
  ...
  await reserveBudget(WS_A, BigInt(80))
  const first = await dbA.crmAiBudget.findFirst({ where: { month: MONTH } })
  assert.ok(first?.alertSentAt, '80% 인데 경보가 없다')

  await reserveBudget(WS_A, BigInt(5))
  const second = await dbA.crmAiBudget.findFirst({ where: { month: MONTH } })
...
test('예상 비용을 미리 잡고, 실제가 나오면 차액만 움직인다', async () => {
  ...
  await reserveBudget(WS_A, BigInt(100)) // 예상 100 선점
  const mid = await dbA.crmAiBudget.findFirst({ where: { month: MONTH } })
  assert.equal(mid?.spentMinorUsd, BigInt(100))

  await settleBudget(WS_A, BigInt(100), BigInt(30)) // 실제는 30
  const after = await dbA.crmAiBudget.findFirst({ where: { month: MONTH } })
...
test('정산이 두 번 들어와도 잔액이 늘어나지 않는다 (음수 방지)', async () => {
  ...
  await settleBudget(WS_A, BigInt(100), BigInt(0))
  await settleBudget(WS_A, BigInt(100), BigInt(0))
  const after = await dbA.crmAiBudget.findFirst({ where: { month: MONTH } })
...
test('★ DI-15 상한을 올리면 즉시 풀린다 — 다음 호출까지 기다리게 하지 않는다', async () => {
  ...
  const b = await setBudgetLimit(WS_A, 'mb_owner', BigInt(1000))
  assert.equal(b.blockedAt, null, '상한을 올렸는데 여전히 막혀 있다')

  // 이제 호출이 된다
  const r = await reserveBudget(WS_A, BigInt(10))
...
test('상한을 내려서 이미 초과했으면 그 자리에서 막힌다', async () => {
  ...
  const b = await setBudgetLimit(WS_A, 'mb_owner', BigInt(100))
  assert.ok(b.blockedAt, '한도를 이미 넘었는데 안 막혔다')

  const e = await catchError(() => reserveBudget(WS_A, BigInt(1)))
  assert.ok(e instanceof CrmError)
  assert.equal((e as CrmError).code, 'BUDGET_BLOCKED')
  await cleanup()
})
...
test('상한 변경은 감사에 남는다 — 누가 언제 올렸는지 답할 수 있어야 한다', async () => {
  await reset(BigInt(100), BigInt(0))
  await setBudgetLimit(WS_A, 'mb_owner', BigInt(9999))
  const audit = await dbA.crmAuditLog.findFirst({ where: { action: 'budget.limit_changed' } })
  assert.ok(audit)
...
test('음수 상한은 거절한다', async () => {
  const e = await catchError(() => setBudgetLimit(WS_A, 'mb_owner', BigInt(-1)))
  assert.ok(e instanceof CrmError)
...
test('★ 상한 0 이면 비용을 모르는 호출도 막힌다 — "AI 끄기"를 뚫으면 안 된다', async () => {
  ...
  const e = await catchError(() => reserveBudget(WS_A))
