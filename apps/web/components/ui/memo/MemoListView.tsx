          ...
          {(['unreviewed', 'all'] as const).map((s) => (
            <button key={s} onClick={() => setStatusFilter(s)}
              style={{
                ...
                color: statusFilter === s ? '#4f46e5' : '#64748b', fontWeight: statusFilter === s ? 600 : 400,
              }}>
              {s === 'unreviewed' ? '미확인' : '전체'}
            </button>
          ))}
              ...
              return (
                <li key={m.id}
                  ...
                  <div style={{ display: 'flex', gap: '0.3rem', flexShrink: 0 }}>
                    ...
                    <button onClick={() => setPromoteTarget(m)} title="업무로 전환" style={iconBtn('#4f46e5')}><ArrowUpRight size={14} /></button>
