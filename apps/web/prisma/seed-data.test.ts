...
test('파이프라인은 TASKS T0-07 이 지정한 4종이다', () => {
  assert.deepEqual(
    SEED_PIPELINES.map((p) => p.name),
    ['GPU 인프라', '파트너십', '공공', 'KDC 제품'],
  )
})
...
test('스테이지 총 개수 25 (7+6+6+6)', () => {
  assert.equal(SEED_PIPELINES.reduce((n, p) => n + p.stages.length, 0), 25)
})
