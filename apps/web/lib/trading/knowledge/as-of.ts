 ...
 *
 * 사람이나 AI 가 정하는 값이 아니다. 정할 수 있게 두면 「이 카드는 그때도 알았던 것으로 해 두자」가
 * 생기고, 그 한 줄이 백테스트 전체를 못 믿게 만든다. 그래서 넣는 자리에서 값을 안 받는다.
 */
 ...
 * 넣을 때 쓸 값. **인자를 안 받는다.**
 *
 * `available_at` 을 밖에서 넘길 수 있게 두면 「그때도 알았던 것으로 해 두자」가 생긴다.
 * 그 한 줄이 백테스트 전체를 못 믿게 만든다.
 */
export function availableNow(): string {
