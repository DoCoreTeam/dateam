...
const DOCUMENT_SURFACES = new Set<string>([
  ...
  // **남이 준 엑셀 원본**을 그대로 편 자리다(v0.10.32x). 목록이 아니라 그 파일의 셀이다 —
  // 정렬 화살표나 쪽 넘김을 붙이면 원본이 아니라 우리가 손댄 표가 되고, 그러면 대조가 안 된다.
  // 브라우저가 엑셀을 못 그려서 우리가 대신 펴는 것뿐이다.
  'components/ui/crm/QuoteOriginalCompare.tsx',
