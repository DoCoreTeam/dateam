...
/**
 * 회의노트에서 **본문 말고 나머지** — 제목·일시·부서·참석자·태그.
 *
 * 왜 이름이 따로 필요한가: 상세 화면에 「수정」이 **둘**이었다. 위쪽은 이 폼을 열고,
 * 원문 카드 안쪽은 본문을 연다. 이름이 같으니 사용자는 어느 쪽이 무엇을 고치는지
 * 눌러 봐야 알았다(사용자 지적 2026-09-17: *"수정버튼이 상단에도 있고 여기도 있는데
 * 정리가 필요할듯 로직상"*). 본문은 작업대 하나가 맡으므로, 위쪽은 이 말로 부른다.
 */
export const NOTE_INFO_LABEL = '정보'
