...
/**
 * 관리자 줄의 말 — **부여 0건이 「아무도 안 들어간다」로 읽히지 않게.**
 *
 * 실측 2026-09-26: 관리자가 접근권한 화면을 열고 「부여가 아직 없어요」를 봤다.
 * 그 표면에 관리자 자신은 이미 들어가고 있었는데도 화면은 그 사실을 한 글자도 안 말했다.
 * 그래서 관리자는 자기에게 부여를 하나 더 만들려 했고, 그건 아무것도 안 바꾸는 줄이다.
 *
 * 판정 1번(`lib/access/decide.ts`)이 관리자를 맨 먼저 통과시킨다 —
 * **그 사실이 화면에 있어야** 부여 목록이 읽힌다.
 */
export const ACCESS_ADMIN_ALWAYS = '관리자는 부여 없이 언제나 지납니다'

/**
 * 그 위에 문이 하나 더 있는 표면(서비스 멤버십·소유자)에서 쓰는 말.
 *
 * 같은 줄에 「언제나」라고만 적으면 바로 위에 붙은 경고와 반대되는 말이 된다 —
 * 두 줄이 서로를 지우면 관리자는 둘 다 안 믿는다.
 */
export const ACCESS_ADMIN_ALWAYS_BUT = '관리자는 부여 없이 지납니다. 다만 위에 적힌 문이 하나 더 있습니다'

/** 이 줄이 무엇인지 — 부여가 아니라 역할에서 나온 줄이다 */
export const ACCESS_ADMIN_SOURCE = '역할'
