...
export function validateSupplierItem(
  ...
  else if (!looksLikeGpuModel(model)) issues.push({ field: 'model_name', severity: 'block', msg: `"${model}" — GPU 모델이 아닌 것 같아 제외(메뉴·라벨 등 오추출 방지)` })
...
export function validateCompetitorItem(
  ...
  const cModel = typeof it.model_name === 'string' ? it.model_name.trim() : ''
  if (!cModel) issues.push({ field: 'model_name', severity: 'block', msg: '모델명 없음' })
  else if (!looksLikeGpuModel(cModel)) issues.push({ field: 'model_name', severity: 'block', msg: `"${cModel}" — GPU 모델이 아닌 것 같아 제외(메뉴·라벨 등 오추출 방지)` })
