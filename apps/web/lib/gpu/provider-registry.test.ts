...
import {
  isCompetitorProvider, detectCompetitorProviders, classifyByIntent, resolveClassification, providerFromUrl,
...
// [verda 실화면 회귀고정 v0.7.356] 화이트리스트 미등록 사이트 → 경쟁사명 공란 사고.
//   verda.com 41건이 competitor_name=""로 저장될 뻔했다. 도메인은 그 자체로 회사 식별자.
test('providerFromUrl — 화이트리스트에 없어도 도메인으로 경쟁사명 확보', () => {
  assert.equal(providerFromUrl('https://verda.com/pricing#gpu-instances'), 'Verda')
  assert.equal(providerFromUrl('https://www.softbank.jp/business/x'), 'Softbank')
  assert.equal(providerFromUrl(''), '')
  assert.equal(providerFromUrl('not-a-url'), '')
})
