...
test('observationToKrwPerGpuHour: per_qty divides before fx/time conversion', () => {
  const obs: AiObservation = {
    ...
    // kind는 시간축 성분(flat)이어야 한다 — base_fee·storage는 v0.7.359부터 대표가 산출 제외.
    // 이 테스트의 목적은 per_qty 나눗셈 순서 검증이므로 kind만 교체(의도 보존).
    component_kind: 'flat', catalog_match: 'A100', match_basis: 'exact', provenance: '1,000円/100GB/月',
  }
  const fx = { JPY: 9.5 }
  const result = observationToKrwPerGpuHour(obs, fx)
  // (1000/100) * 9.5 / 720
...
// [실화면 회귀고정 v0.7.359] 계정 고정비·용량요금은 GPU 시간단가가 아니다.
//   실측: 소프트뱅크 기본료 30,000엔/월이 $0.257 "시세"로 표시돼 A100 항목이 5건으로 부풀고
//   시장 밴드까지 끌어내렸다. GPU를 몇 장 쓰든 고정인 요금을 GPU 1장 시간당으로 나누면 의미가 없다.
//   성분으로는 보존한다(무손실) — 실효비용 합산은 scenario-cost 담당.
test('base_fee는 대표가(GPU 시간단가) 산출에서 제외 — null', () => {
  const base = {
    competitor_name: 'SoftBank', model: 'A100', form_factor: null, memory_gb: null, gpu_count: 1,
    amount: 30000, currency: 'JPY', unit: 'month', per_qty: 1,
    catalog_match: 'A100', match_basis: 'exact', provenance: 'A100 月額基本料金 30,000円',
  }
  const r = validateAiObservation({ ...base, component_kind: 'base_fee' })
  assert.equal(r.ok, true)
  if (r.ok) assert.equal(observationToKrwPerGpuHour(r.value, { JPY: 9.1327 }), null, '기본료는 시간단가 아님')
})

test('storage(용량요금)도 대표가 제외 — null', () => {
  const r = validateAiObservation({
    competitor_name: 'SoftBank', model: 'A100', form_factor: null, memory_gb: null, gpu_count: 1,
    amount: 1000, currency: 'JPY', unit: 'per_gb', per_qty: 100, component_kind: 'storage',
    catalog_match: 'A100', match_basis: 'exact', provenance: 'A100 データストアストレージ 1,000円/100GB',
  })
  assert.equal(r.ok, true)
  if (r.ok) assert.equal(observationToKrwPerGpuHour(r.value, { JPY: 9.1327 }), null)
})

test('usage(GPU 종량)는 정상 산출 — 제외 규칙이 과하지 않음', () => {
  const r = validateAiObservation({
    competitor_name: 'SoftBank', model: 'A100', form_factor: null, memory_gb: null, gpu_count: 1,
    amount: 7.2, currency: 'JPY', unit: 'minute', per_qty: 1, component_kind: 'usage',
    catalog_match: 'A100', match_basis: 'exact', provenance: 'A100 GPU利用料金 7.2円/1分',
  })
  assert.equal(r.ok, true)
  if (r.ok) {
    const krw = observationToKrwPerGpuHour(r.value, { JPY: 9.1327 })!
    assert.ok(Math.abs(krw - 7.2 * 60 * 9.1327) < 1e-6, `기대 ${7.2 * 60 * 9.1327}, 실제 ${krw}`)
  }
})
