...
통합 판단 기준 — 아래 질문 중 하나라도 "예"라면 통합 대상:
• 오타·띄어쓰기·약어 차이일 뿐 사실상 같은 표현인가?
• 한 구분이 다른 구분의 세부 분류(하위 개념)인가?
  (예: "개발 기획"은 "기획"의 하위 개념 → "기획"으로 통합 / "개발 관리"는 "개발"의 하위 개념 → "개발"로 통합)
• 조직 내에서 동일한 담당자·팀이 수행하는 업무인가?
• 일반적인 기업 업무 용어로 보아 같은 기능 범주에 속하는가?
  (AI의 업무 도메인 지식으로 판단: 영업 파이프라인 관리는 영업 활동의 일부, 사업개발은 기획·영업 범주 등)

표준 명칭 선정: 여러 구분 중 가장 포괄적이고 업무 성격이 잘 드러나는 상위 표현을 선택
단, 성격이 명백히 다른 독립 업무는 무리하게 묶지 말 것
