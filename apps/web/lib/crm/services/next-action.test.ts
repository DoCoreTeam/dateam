...
test('★ 보드 카드가 실제로 다음 할 일을 그린다 — 안 그리면 정적인 목록 그대로다', () => {
  ...
  /*
    **경고가 아니라 «할 수 있는 것»이어야 한다.**
    예전엔 「다음에 뭘 할지 정해 주세요」라고 말만 했다 — 부탁일 뿐 어디서 정하는지
    알려 주지 않아 사용자가 상세로 들어가 할 일 칸을 찾아야 했다
    (사용자 지적 2026-08-28: 「다음 할일 작성 버튼을 띄우자」).
    이제 그 자리가 버튼이고, 누르면 상세의 그 칸으로 커서까지 간다.
  */
  assert.ok(BOARD.includes('다음 할 일 적기'), '계획 없는 딜에 «적을 길»이 없다')
  assert.ok(BOARD.includes('#crm-next-task'), '버튼이 할 일 칸으로 데려가지 않는다')
