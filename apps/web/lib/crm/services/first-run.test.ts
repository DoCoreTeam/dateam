...
/**
 * **말과 기준이 함께 바뀌었다**(2026-09-09).
 *
 * ① 말: 여기서 접는 것은 «파이프라인»이지 «영업 단계»가 아니다. 영업 단계는 그 안의
 *    칸(리드 → 제안 → 성사)이다. 앱의 나머지 전부가 「파이프라인」을 쓰는데 이 자리만
 *    틀린 말을 쓰고 있었다(사용자 지적: 「영업단계가 오히려 영업단계가 아니라 파이프라인」).
 * ② 기준: 예전엔 「딜 0건이면 안 쓰는 것」이라고 **추측**했다. 지금은 설정에서 접은
 *    사실(`isActive`, 마이그 245)을 본다. 추측이 사실이 됐다.
 */
test('★ 리포트가 안 쓰는 파이프라인을 접는다', () => {
  assert.ok(REPORTS.includes('showEmpty'), '접기 상태가 없다')
  assert.ok(REPORTS.includes('아직 안 쓰는 파이프라인'), '접었다는 사실을 안 알린다')
  assert.ok(REPORTS.includes('isActive'), '접힘을 설정이 아니라 딜 수로 추측하고 있다')
})
...
test('★ 딜 보드 선택은 접지 않는다 — 방금 만든 파이프라인을 «안 쓰는 것»이라 부르지 않는다', () => {
  ...
  //
  // v0.7.692: 예전 판은 세는 **식**(`dealCount ?? 0`)을 그대로 찾았다. 그런데 그 식이
  // 화면에 두 벌로 흩어져 있던 것이 「전체 (7개)」 사고의 구조적 원인이라 SSOT 로 모았고,
  // 그러자 이 가드가 멀쩡한 코드를 실패로 만들었다 — 바로 아래 시험이 경고하는 그 함정이다.
  // 지켜야 할 것은 식이 아니라 **파이프라인마다 딜 수가 적혀 있는가**다.
  assert.ok(
    /dealsInPipeline\(p\)/.test(SHELL) || /dealCount \?\? 0/.test(SHELL),
    '파이프라인마다 딜 수를 안 적는다 — 비어 있다는 사실을 숨기게 된다',
  )
...
test('접은 것을 숨기지 않는다 — 몇 개인지는 말한다', () => {
  assert.ok(/안 쓰는 파이프라인 \{unused\.length\}개/.test(REPORTS), '개수를 안 밝힌다')
})
