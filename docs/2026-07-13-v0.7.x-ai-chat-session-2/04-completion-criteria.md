...
## DC-REV 통합 리뷰 반영 (commit 5e9e94f)
- **[블로킹] 편집분기 표시 버그**: `buildActiveThread` 이중적용(서버 축약본 + 클라 재적용)으로 새로고침/SSR 시 편집 질문 소실 → **수정**. 클라가 `messages`를 활성스레드로 직접 유지(편집 낙관갱신=인덱스 직접 절단), 렌더 재적용 제거. 회귀 테스트 추가(thread 비멱등성 문서화).
- **[복원] 재생성 실패 데이터 손실**: 실패 시 기존 응답 덮어쓰기 제거 → 보존(§5-1). 클라는 재조회로 복원.
- 재검증: tsc exit0 · ai-chat 55/55.
