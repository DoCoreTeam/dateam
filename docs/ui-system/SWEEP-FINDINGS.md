...
- **`/pricing/gpu?tab=board`의 구뷰(PriceTableTab) 모달** — '파트너 등급 관리'.
  가격표 탭은 통합 표(`unified` 플래그 기본 ON)가 그리므로 그 구뷰가 기본 경로가 아니다.
  셸 이관은 했으나 눈으로 못 봤다.
- **`/ci/briefs/[id]` · `/ci/channels/[id]`** — 목록 라우트가 없고(둘 다 404) 링크를 만들 데이터가 없어 도달하지 못했다.
- **모달 전부의 모바일 확인** — GPU 외 화면의 생성·편집 모달은 데스크톱만 봤다.
- **E2E 34종** — 실데이터를 변경할 수 있어 이번 훑기에서 돌리지 않았다.
