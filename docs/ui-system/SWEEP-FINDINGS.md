...
**검사기가 스스로 눈을 가린 사고 — `networkidle`.** 처음엔 각 화면에서 `waitForLoadState('networkidle')`을
기다렸다. `/ci/*`는 큐 구동기가 주기적으로 요청을 보내 **idle이 영영 오지 않는다.** 결과는
`31 통과 / 15 실패(28.5분)`였는데, 실패 15건이 전부 타임아웃이라 **치수는 재 보지도 못했다.**
"CI 화면에 문제가 있다"가 아니라 "검사기가 CI 화면을 못 열었다"였다.
대기 조건을 `main.page-inner`가 보이는지로 바꾸자 **46/46 통과(7.7분)**. 초록도 빨강도 못 믿는 상태였던 것이다.

<!-- 최종: v0.7.486 기준 46/46 통과 · 스킵 0 · 7.7분 -->

**폴링이 도는 화면을 검사할 때 `networkidle`을 쓰지 않는다.** 다른 E2E를 새로 쓸 때도 같다.
