...
| `ui/doc/DocSurface` | **1** | **고객에게 나가는 문서의 «종이»** — 셸을 덮는 미리보기(포털) + 도구 줄 + A4 종이 + 인쇄 규칙. 엑셀·PDF·이미지가 전부 이 안에서 나간다. 화면을 그대로 인쇄하면 사이드바·상단 바·회색 배경이 고객 PDF 에 찍힌다(v0.7.643) | **표준** — §2-7, `doc-export-standard` 가드가 잠금 |
| `ui/SectionSurface` | **1** | **한 덩어리를 «면»으로 묶는다** — 머리띠(제목·건수·금액·단서) + 본문. `bleed` 는 스스로 여백을 갖는 것(보드 컬럼·표)을 넣을 때. 딜 보드가 파이프라인을 **제목 글자로만** 나눠 어디서 끊기는지 안 보이던 것을 고치며 신설(v0.7.639) | **표준** — 화면에서 테두리·머리띠를 손으로 그리지 않는다 |
...
| 방식 | 건수 | 판정 |
|---|---|---|
| `ui/nb/NbButton` | **85** | 표준 — `href` 를 주면 `<a>`(새 탭·우클릭·키보드 동작). `<Link className="btn-ghost">` 자작 금지 |
...
### 5-2-1. 되돌릴 수 없는 삭제 확인
| 부품 | 사용 | 판정 |
|---|---|---|
| `ui/ConfirmDeleteDialog` | **5** (v0.7.494 신설) | 표준 — CI 삭제 5화면 |

CI의 삭제는 **물리 삭제**라 되돌리기가 없다(사용자 결정 2026-08-16).
안전장치가 "누르기 전" 한 번뿐이라 확인창을 화면마다 자작하면 편차가 곧 데이터 소실이 된다.
무엇이 함께 사라지는지(서버가 센 수)와 무엇이 남는지를 **둘 다** 보여 준다 —
남는 것을 안 밝히면 사용자가 다 없어지는 줄 알고 못 지운다.
흐름은 `lib/ci/use-delete.ts`(미리보기 → 확인 → 삭제)가 SSOT다.
가드: `lib/ci/delete-contract.test.ts`.
