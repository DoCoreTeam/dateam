...
**한자를 쓰지 않는다 (필수 — 보고서·화면·주석 전부)**

> **왜**: 층위 기획 보고서에서 장 표식에 기승전결을 한자로 썼다. 문서를 격식 있게 보이게
> 하려는 취향이었지 **이 조직의 문서 문화가 아니다.** 읽는 사람이 뜻을 한 번 더 번역해야
> 하는 표기는 격식이 아니라 **장벽**이다.
> (실제 지적 2026-09-05: **"앞으로 보고서 만들때 절대 한자를 넣지마라 우리는 한자를 쓰는
> 문화가 아니다 내가 지시할때 아니면 절대 한자 금지"**)
>
> 게다가 한글로 쓰면 **같은 자리에서 더 정확해진다** — 「기」보다 「증거」가, 「전」보다
> 「계획」이 그 절에 실제로 무엇이 들어 있는지 더 잘 말한다. 한자는 뜻을 압축하는 대신
> **무엇을 압축했는지 감춘다.**

- **화면·문서·보고서·커밋 메시지·주석 어디에도 한자를 쓰지 않는다.** 주석처럼 사용자에게
  안 보이는 자리도 마찬가지다 — 「안 보이니까 괜찮다」는 예외가 아니다.
- **유일한 예외는 사용자가 그 자리에 한자를 쓰라고 지시했을 때다.** 원문에 한자가 있는
  고유명사·인용문은 원문이므로 고치지 않는다.
- 기승전결 같은 구성을 쓸 때는 그 절이 **담는 것**을 한글로 이름 붙인다 —
  증거 · 원인 · 계획 · 화면 · 규칙 · 빈틈 · 승인.
- 확인: `grep -n '[一-鿿]' <파일>` 이 **0건**이어야 한다. 가드는 `lib/ui/glossary.test.ts`.
...
| 카드 | **`className="card"`** (클래스) | 196 | ~~`NbCard`~~ (v0.7.445 삭제 — 0건이었다), 자작 박스(476건) | — 여백은 `--card-pad` 가 준다(§2-3-4). 전면형만 `card-flush`
...
### 2-3-4. 공용 부품은 **자기 몫의 여백을 스스로 갖는다** (필수)

> **왜**(실측 v0.7.676, 사용자 지적): 「모든 걸 구현할때마다 AI웹 검색 한도 저쪽 영역처럼
> 디자인을 다 깨버리네? **왜 계속 동일한 이슈야 앞으로 절대 만들지마**」
>
> `/ci/trends?tab=signals` 의 수집 상태 줄이 **computed padding 0px** 로 렌더돼 글과 버튼이
> 카드 가장자리에 붙어 있었다. 그런데 그 화면은 규칙을 어긴 적이 없다 — `.card` 를 썼고
> 토큰도 지켰다. 원인은 **부품**이었다: `globals.css` 의 `.card` 가 배경·모서리·그림자·테두리는
> 정하면서 **안쪽 여백만 안 정해서**, 여백을 **174개 호출처 전부**에게 떠넘기고 있었다.
> 실측 — 손으로 적은 곳 **145곳에 값이 11가지**, 아예 안 적은 곳 **29곳**.
> 안 적으면 조용히 깨지고 `tsc`·`lint`·`design:check` 어느 것도 그 누락을 못 봤다.
>
> 더 나쁜 것은 **「여백 없음」이 두 가지 뜻을 겸했다**는 점이다 —
> ① 전면(full-bleed)형이다(안에 자기 여백을 가진 머리줄·표가 들어간다) ② 적는 걸 잊었다.
> 같은 신호가 둘을 뜻하니 사람도 기계도 구분할 수 없었다. **그 모호함이 재발의 뿌리다.**

| | 규칙 |
|---|---|
| **C-1** | **부품이 기본값을 갖는다.** 화면이 안 적어도 제대로 그려져야 한다 — 카드 여백은 `--card-pad` 가 정하고 `:where(.card)` 가 적용한다 |
| **C-2** | **기본값은 특정도 0(`:where`)으로 준다.** 이미 자기 값을 적어 둔 화면이 전부 그대로 이겨야 **무회귀**다 |
| **C-3** | **예외는 말로 밝힌다.** 전면형은 `className="card card-flush"`. 인라인 `padding: 0` 으로 «잊은 것»과 같아 보이게 두지 않는다 |
| **C-4** | **새 공용 부품을 만들 때 같은 질문을 한다** — 「이 부품을 쓰는 화면이 **무엇을 안 적으면 깨지는가**」. 그것이 있으면 그건 부품이 가져야 할 기본값이지 화면의 숙제가 아니다 |

- **가드**: `lib/ui/card-standard.test.ts` — ①부품이 기본 여백을 잃음 ②`.card-flush` 소실 ③화면이 인라인 `padding: 0` 으로 되돌림 ④`card-flush` 단독 사용. **만든 뒤 4가지를 일부러 깨서 전부 확인했다.**
- **적용 범위는 카드만이 아니다.** 「화면이 잊으면 깨지는 값」을 부품 밖에 두는 것 전부가 같은 부류다.
...
**(5) 읽으러 온 화면은 읽기로 연다. 그리고 같은 값을 쓰는 길은 하나다** (v0.7.677)

> **왜**: 회의노트 상세를 열면 본문에 **곧바로 커서가 들어갔다.** 그런데 같은 화면 위쪽은
> 「[수정]에서 추가하세요」라고 말하고 있어서, 한 화면이 «읽는 곳»과 «쓰는 곳»을 동시에 주장했다
> (사용자 지적: "수정을 눌러야 텍스트 수정이 되는 에디터 모드여야 하지 않아?").
> 자동저장이라 끝난 회의를 다시 읽다 글자가 지워져도 5초 뒤 저장돼 되돌릴 곳도 없었다.
>
> 더 깊은 것은 그 다음이다. 그 화면에는 **본문을 쓰는 길이 둘**이었다 —
> 작업대(자동저장 `PATCH`)와 「수정」 폼(명시 저장). 작업대는 저장 뒤 `router.refresh()` 를
> 하지 않으므로 서버 컴포넌트가 준 `note.body` 는 **페이지를 연 시점**에 머문다.
> 회의 중에 작업대로 적고 나서 「수정」 폼을 열어 저장하면 그 글이 **옛 내용으로 조용히 덮였다.**
> 오류도 경고도 없다. 이건 UI 불일치가 아니라 **데이터 유실**이다.

- **읽기가 기본, 쓰기는 명시.** 상세 화면의 본문·설명 같은 «읽는 것»은 읽기 컴포넌트로 그리고,
  `ACTION.edit` 버튼을 눌러야 편집기가 열린다. **예외는 적을 것이 아직 없을 때뿐**(빈 본문 →
  바로 쓰기. 읽을 게 없는 화면에 관문을 두면 받아적기를 막는다).
- **한 값에 쓰는 길은 하나.** 불가피하게 둘이면, **나중에 여는 쪽이 최신을 다시 읽는다.**
  못 읽으면 **열지 않는다** — 옛 값으로 여는 것이 곧 그 사고이므로, 여기서 «그냥 열어 주는»
  관대함은 데이터를 잃는 쪽이다. 실패는 화면에 말한다(`failedTo`).
- **모드 판정은 컴포넌트 밖으로.** `useState(() => …)` 안의 식은 실브라우저 외에 검증 수단이 없다(E-6).
  `lib/meeting/memo-mode.ts` 처럼 순수 함수로 빼고 가드로 잠근다(만든 뒤 세 갈래를 일부러 깨서 확인했다).
...
### 2-3-6. 층위의 축 — 무엇이 형제이고 무엇이 자식인가 (필수)

> **왜**: 부품(§0)·말(§0-2)·자리(§2-3-2)·길(§2-3-3)을 다 정해도 **층위가 뒤섞이면 다른 제품이다.**
> 실측(v0.7.688) — 회의 작업대가 「작성 · 녹음·전사 · 정리」를 **같은 높이의 탭 셋**으로 세웠다.
> 그런데 셋은 형제가 아니다. 앞의 둘은 **원문**이고 정리는 **그 둘을 읽어서 만든 결과**다.
> (사용자 지적 2026-09-05: *"결국 우리가 봐야되는건 정리된 내용일텐데 … 정리가 가장 먼저
> 보여야 하는거 아냐? 그리고 그러다 보니깐 이런식으로 동일 레벨의 탭으로 있는게 이상한데"*)
>
> 자식을 형제 자리에 세우면 **세 가지가 동시에 깨진다.** 그리고 실제로 셋 다 깨져 있었다:
>   ① **기본을 못 정한다.** 결과를 열면 원문이 사라지고 원문을 열면 결론이 숨는다.
>      그래서 코드가 「정리는 기본값이 되지 않는다」로 회피했다 — 그 회피 코드가 증거였다.
>   ② **상태를 표시할 자리가 없다.** 탭 라벨은 분량을 적는 자리인데(218자·406줄) 결과물에
>      필요한 것은 **있음·없음·낡음**이다. 「방금」이라는 임시 표시밖에 못 붙였다.
>   ③ **만드는 버튼이 탭 안에 갇힌다.** 그래서 탭 밖에 버튼을 하나 더 만들었고, 두 버튼이
>      **서로 다른 곳에 쓰기 시작했다.** 실측 16건이 그 결과다 — 정리가 저장됐는데 화면
>      어디에도 안 나왔다(한쪽은 `meeting_notes.summary`, 다른 쪽은 `meeting_note_digest`).

| | 규칙 |
|---|---|
| **P-1** | **탭은 「둘 중 하나를 고른다」가 성립할 때만 쓴다.** 재료에서 나온 결과물을 재료와 같은 줄에 세우지 않는다. 결과물은 **카드 본문**, 재료는 **접기** |
| **P-2** | **새 내비게이션 장치를 만들지 않는다.** 탭은 그대로 두고 **줄인다** — 진짜 형제끼리만 남긴다. 주소(`?tab=`)는 살려서 기존 링크가 죽지 않게 한다 |
| **P-3** | **결과물이 위, 재료가 아래.** 다만 **결과물이 아직 없으면 재료를 펼친다** — 그때 할 일은 «적는 것»이고, 접힌 화면에서는 적을 자리가 한 번 더 눌러야 나온다 |
| **P-4** | **한 결과물을 만드는 자리는 하나다.** 버튼이 둘이면 결국 **다른 곳에 쓴다.** 만드는 버튼은 그 결과물 곁에 두고, 다른 화면에서는 **그리로 안내만** 한다 |

- **판정 기준은 «무엇을 읽어서 만들었나»다.** A와 B를 읽어야 C가 나오면 C는 A·B의 형제가 아니다.
  같은 부류: 원문 → 요약 · 자료 → 분석 · 항목 → 합계 · 후보 → 확정본.
- **층위 판정은 컴포넌트 밖으로 뺀다**(완료 조건 E-6). `useState(() => …)`·`useEffect` 안의 식은
  실브라우저 외에 검증 수단이 없다. 순수 함수로 빼고 가드로 잠근다
  (예: `lib/meeting/workbench-tab.ts` 의 `pickEvidenceTab`·`evidenceOpenByDefault`).
- **접기는 `<details>` 로 한다** — 열고 닫는 키보드·스크린리더 규약이 브라우저에 이미 있다.
  자작 토글은 그것을 다시 만들어야 하고, 대개 빠뜨린다. **접혀 있으면 본문을 그리지 않는다**
  (`<details>` 는 닫아도 자식을 문서에 두므로, 그냥 두면 열지도 않은 것을 매번 내려받는다).
- **가드**: `lib/meeting/workbench-tab.test.ts` — ①결과물이 탭으로 되돌아옴 ②접힘 렌더 가드 소실
  ③결과물이 재료보다 아래로 감 ④판정이 SSOT 를 안 거침. **만든 뒤 일부러 깨서 확인한다.**
...
## 배포본 정합성 정책 (필수 — 위반 시 "로컬은 되는데 프로덕션만 죽는다")

> **왜**: 주간보고 저장이 **2주 동안 프로덕션에서만 100% 실패**했다(실측 2026-08-31).
> 코드는 맞았다. `next.config.js` 의 `serverComponentsExternalPackages` 에 `sanitize-html` 이
> 올라가 있어 webpack 이 **번들에 넣지 않았고**, 배포본에도 그 파일이 실리지 않아
> 모듈 로드 자체가 실패했다. 저장 POST 가 인증 왕복 전 73~293ms 만에 500 이었다.
> **tsc·단위테스트·design:check 는 전부 초록이었다** — 코드 검사로는 영원히 못 잡는다.
> 그리고 그 실패가 **화면에도 로그에도 한 글자도 안 남아** 버튼이 영원히 「저장 중…」이었다.
...
| **B-7** | **몇 초를 넘게 기다리게 하는 일은 «무엇을 하는 중인지»와 «얼마나 지났는지»를 말한다.** 「읽는 중…」 한 마디로 3분을 덮으면 사람은 진행이 아니라 **고장**으로 읽는다(실측 v0.7.684·2026-09-09·2026-09-20, 같은 지적 세 번). 문구는 `lib/meeting/digest-progress.ts` 가 SSOT 이고 화면은 `components/ui/WaitProgress` 로 그린다 — 문턱(2.5·45·120초)을 화면마다 다시 정하면 같은 일에 다른 말이 나온다. **진행 표시는 결과 유무 분기 밖에 둔다**(분기 안에 두면 첫 실행이 통째로 침묵한다) |
...
- **가드**: `lib/ui/deploy-fragile.test.ts` 가 B-1~B-6 을 전부 검사한다(만든 뒤 **일부러 깨서** 6개 모두 확인했다). B-7 은 `lib/policy/wait-progress-guard.test.ts` 가 창구의 `maxDuration` 에서 대기 자리를 뽑아 센다.
