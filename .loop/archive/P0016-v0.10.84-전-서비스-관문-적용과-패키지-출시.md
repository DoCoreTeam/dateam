...
- pnpm tsc --noEmit: 내 범위 오류 0 (남은 것은 옆 세션의 app/admin/members/[id]/EmploymentCard 미생성)
- pnpm lint: 오류 0
- pnpm typecheck:packages: 패키지 넷과 그 시험까지 통과
- pnpm test: 5,748 통과 / 실패 2 (둘 다 옆 세션 파일 — EmploymentCard 날짜 입력, DeleteNodeModal 전각 대시)
- pnpm build: Compiled successfully
- Playwright: 개발자센터 AI 공통층 네 항목 실브라우저 통과

완료 정의 대조

- AI 를 부르는 모든 길이 관문을 지난다: 벤더 주소를 든 파일 25 -> 6, 여섯 전부 「관문안쪽」에 사유 한 줄, 사유 없는 것 0개를 가드가 센다
- 확신을 그리는 화면이 전부 공용 규칙을 지난다: 기준선 25 -> 0 (재측정 결과 실제로 그리던 것은 열셋)
- 패키지 넷이 이름과 판과 라이선스와 읽을 문서를 갖춤: 판 0.1.0, 사내 독점 라이선스, README 다섯
- 쓰는 법 문서 하나로 새 서비스가 이 층을 붙일 수 있음: packages/README.md 와 개발자센터 「AI 공통층」 메뉴 둘 다
- 문서와 코드가 갈리면 시험이 막음: package-docs-guard 6단정, ai-layer-docs-guard 7단정

발견 사항

- 이관을 막던 것은 규칙이 아니라 손으로 원장과 이름 목록을 내려보내는 수고였다, 진짜로 적는 기본값을 두자 스물넷이 풀렸다
- 원장 클라이언트를 못 만들면 AI 호출이 전부 죽는 길이 있었다, 「기록 실패가 호출을 막지 않는다」를 표 오류만 지키고 있었다
- 가드 판정이 틀려 멀쩡한 파일을 잡던 것 둘, 반대로 잠들어 한 번도 안 돌던 것 하나
- 문서를 먼저 쓰고 서명을 안 본 예제 셋, 지어낸 능력 이름 여덟

범위 밖 변경

- 남의 세션 파일은 커밋에서 뺐음, 다만 apps/web/package.json 의 가드 등재 한 줄이 남의 커밋(v0.10.109~111)에 실려 나갔고 scripts/.design-guard-baseline.json 은 한 파일이라 남의 하향분과 함께 커밋됐음
- v0.10.116 커밋은 기준선 한 줄만 바꾸면서 버전 여섯 파일을 안 올렸음, 다음 커밋이 0.10.119 로 맞춰 흡수됨
