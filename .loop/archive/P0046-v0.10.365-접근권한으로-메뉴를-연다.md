...
## 종합 감사
2026-09-22, 시작 커밋 d6b032ea 기준

### 1 검사 넷
- `pnpm tsc --noEmit` 통과 (출력 없음)
- `pnpm lint` 오류 0 (경고는 이 판 이전부터 있던 것)
- `pnpm test` **6654/6654 통과, 실패 0** (판 시작 시 6612 → 42개 늘었다)
- `NEXT_DIST_DIR=.next-p0046 pnpm build` 성공, 격리 dist 는 지운다
- `pnpm design:check` 통과 (hex 0, 기준 초과 0)

### 2 완료 정의 한 줄씩
- 검사 넷 통과 — 위와 같다
- **부여 0건이면 지금과 같다** — 일반 사용자 사이드바 7개로 이 판 앞뒤 동일, 관리자 11개 동일.
  전체 메뉴만 15 → 8 로 줄었는데 그 일곱이 **라우트가 막고 있던 죽은 문**이고, 없애는 것이
  I08 의 감사 기준이었다(v0.1.4 에 적은 의도된 차이)
- **등재를 잊으면 가드가 실패한다** — `app/(member)/temp-unregistered` 를 만들어
  「등재부에 없는 화면: /temp-unregistered」 로 실패를 확인하고 지웠다
- **부여를 저장하면 사이드바와 전체 메뉴가 함께 바뀐다** — 개발본부 소속 일반 사용자로
  사이드바 7→9, 전체 메뉴 8→10 실측(I12)
- **env 추가 없이 DB 저장과 UI 관리** — `.env.example` 변경 0줄, `lib/access/**` 에 `process.env` 0건

### 3 전체 diff
`git diff d6b032ea..HEAD --stat` 101파일 5,719추가 311삭제 — 다른 세션 것(견적 비고·주간 이력·마이그 276)이 섞여 있고
이 플랜 몫은 `lib/access/**`·`app/admin/access/**`·`app/api/admin/access/**`·서비스 셸 넷·내보내기 창구 여덟·마이그 277 이다.
추가된 줄에 비밀 0건, 한자 0건

### 3a 보안 재측정 (7절 「기계가 세는 것」, 운영 DB)
| 세는 것 | 결과 |
|---|---|
| RLS 꺼진 public 표 | **0** |
...
### 4 항목 대 결과
플랜이 「신규」라고 적은 파일 16개가 전부 있다 — 등재부·판정·읽기·순수부·게이트·동작·값과범위·
가드 둘·화면 셋·창구·안내 부품·말·마이그 277

### 5 발견 사항
- **사람 목록이 조용히 비어 있었다** (I07~I11): `profiles` 에 없는 `email` 칼럼을 골라 읽고 있었고
  supabase-js 는 그것을 던지지 않고 `{data:null,error}` 로 돌려준다. `data ?? []` 만 보던 탓에
  오류 한 줄 없이 빈 목록이 그려졌다. I11 에서 고치고 조회마다 `error` 를 보는 `rows()` 를 세웠다
- **부여를 줘도 서비스 문이 안 열렸다** (사용자 보고 ins_0078): 서비스 넷은 자기 멤버십 장치가
  따로라 메뉴만 뜨고 막혔다. I11a 에서 셸 넷에 판정을 잇고, 영업 CRM 은 부여 저장이 자리까지 만들게 했다
- **아직 못 여는 서비스 둘**: 콘텐츠 인텔리전스(워크스페이스 필요)·AI 스튜디오(표 잠금이 관리자 기준).
  화면이 `needsMembership` 으로 그 사실을 저장 전에 말한다 — 조용히 안 열리지 않는다
- **첫 부여는 안 남겼다**: 접근권한 코드가 배포 전이라 부여는 프로덕션에서 아무 일도 안 하지만
  `crm_member` 자리는 바로 열린다. 기능이 나가기 전에 12명을 CRM 에 들여보내지 않는다 —
  배포 뒤 관리자가 화면에서 저장하면 자리도 함께 생긴다(I11a 에서 실측)
