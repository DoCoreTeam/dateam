...
실브라우저 확인 (2026-09-22, 해결)

세션은 사람 로그인 없이 갱신됐음 — auth-state.json 의 쿠키 자체는 10/22 까지 살아 있었고
안에 든 access_token 만 만료였음. GoTrue grant_type=refresh_token 으로 갱신해 쿠키를 다시 씀
(계정을 새로 만들지도, 비밀번호를 넣지도 않음)

- 격리 dev 서버(:3105, NEXT_DIST_DIR)에서 딜 목록 9건 조회 — 로그인 상태 확인
- 딜 상세 → 새 견적 → 파일로 채우기 → quote-sheet.xlsx 업로드
- 결과: 「quote-sheet.xlsx 에서 3개를 읽었어요」, 빨간 AI 오류 문구 없음
- 증거: artifacts/quote-fill/03-review.png

이 플랜 밖에서 발견한 것 (별건, 새 플랜 감)

- 검수 화면 세 줄 모두 「단가를 못 읽었어요」로 읽은 금액 0원 (문서 합계 18,392,000원)
  원문 줄에는 `| 1 | 6050000` 이 그대로 보이므로 글자는 뽑혔고 단가로 옮기는 자리에서 빠진 것
  안전하지 않은 줄은 체크가 꺼지는 것이 설계라 「체크한 항목 넣기 (0)」이 잠기고 e2e 가 거기서 죽음
  화면 결함이 아니라 추출 품질 문제
- 같은 파일을 창구로 한 번 더 부르면 60초 어댑터 상한에 걸려 400 이 남 — 느린 날이 있음
