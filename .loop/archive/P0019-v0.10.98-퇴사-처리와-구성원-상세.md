...
실행 명령과 결과 (2026-09-17)

- pnpm tsc --noEmit: 통과
- pnpm test: 5752개 전부 통과 (플랜 시작 전 5709개, 이번에 더한 것 43개)
- pnpm lint: 오류 0, 경고는 전부 남의 파일의 기존 exhaustive-deps
- pnpm build: 성공, /admin/members 151kB · /admin/members/[id] 109kB 로 새 길이 실제로 잡힘
- node scripts/check-design-tokens.mjs: 통과, 내 파일 위반 0
- Playwright 10개 통과 (member-resign 2 · org-person-link 2 · org-tree-grid 1 · org-delete-transfer 2 · picker-excludes-resigned 1 · 세션 저장 1 · 조직도 한 장 별도)

완료 정의 대조

- 사용자 노출 문자열은 lib/terms/member.ts 를 지남, 화면에 직접 적은 재직 관련 문구 없음
- env 신규 0개, 설정값 추가 없음
- 퇴사 처리 뒤 profiles 행이 남고 deleted_at 이 null 임을 일회용 계정으로 실측 확인
- 실브라우저에서 퇴사 처리와 되돌리기가 동작함

전체 diff (8ecafccc..HEAD, 내 경로만): 24파일 1655줄 추가 74줄 삭제
- 비밀 검사: 키 토큰 비밀번호 0건
- 한자 검사: 0건
- 범위 밖 변경: 없음. 다만 감사에 쓴 Playwright 스펙 다섯을 남겼음 (지우면 다음 판에 같은 것을 손으로 다시 해야 함)

DB 상태
- 마이그 255 257 적용됨, v_member_employment_status 뷰와 함수 둘 존재
- member_employment 0행 (아직 아무도 퇴사 처리 안 함, 정상)
- 검사에 쓴 일회용 계정 0개 일회용 조직 0개 — 실데이터 오염 없음

발견 사항

1. 마이그 번호 256 을 옆 세션이 먼저 써서 257 로 잡았음
2. 조직도 탭에 예전부터 있던 콘솔 오류 `document is not defined` 가 HEAD 에서도 그대로 남 (내 변경과 무관, 화면 동작에는 영향 없음, 별건으로 잡아야 함)
3. 종합 감사에서 pnpm build 를 돌리다 돌고 있던 dev 서버의 .next 를 덮어 화면이 빈 채로 떴음. dev 서버를 다시 띄워 복구했고 전 검사 재통과 확인. **dev 가 돌고 있을 때 build 를 돌리지 않는다**
4. 가드가 내 결함 둘을 잡았음: EmploymentCard 가 raw input type=date 를 썼고(DateField 로 교체), DeleteNodeModal 문구에 전각 대시가 있었음(제거)
