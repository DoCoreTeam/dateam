...
- 실제 견적서 파일 올리기가 성공한다: **실브라우저로 확인함.** 사용자가 본 그 모달(「파일에서 견적 가져오기」)에
  PDF 를 올려 「quote.pdf 에서 2개를 읽었어요」와 합계 9,800,000원, 도착지 고르기까지 떴다. 24.4초, 한도 문구 없음
  - 먼저 코드 경로로도 쟀다: streamChatWithKeys + PDF 첨부, 키 교체 3회 기본 → pickup ai → ai team → 유료키, 25.6초에 품목 2건 JSON
  - 함정 둘: 돌던 :3100 은 **옛 빌드를 띄운 next start** 였다(화면 판 번호 v0.10.255). 지금 코드로 따로 빌드해 :3200 에 세워야 잼
    그리고 패치노트 모달이 클릭을 삼켜 「단추가 없다」로 5분을 죽였다 — localStorage 에 본 판을 심어 지났다
