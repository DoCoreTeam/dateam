...
실행 명령과 결과 (2026-09-16)

- `pnpm tsc --noEmit` 오류 0
- `pnpm lint` 오류 0 (기존 경고 1건 `useTour.ts` 는 이 판 밖)
- `pnpm test` 5591/5591 통과 (판 시작 시 5559 → 가드 32개 늘었다)
- `pnpm build` Compiled successfully (dev 서버를 멈추고 돌린 뒤 되살림)

완료 정의 대조

- 네 명령 전부 통과 — 충족
- 사용자 노출 문자열: 단계 문구는 `FINISH_STEP_TEXT` SSOT 재사용, 진행 문구는 기존
  `finish-progress` 그대로. 화면이 새로 지은 문장 0 — 충족
- 설정값 env 추가 0 (기존 `CRON_SECRET`/`CI_WORKER_TOKEN` 재사용) — 충족
- 항목마다 가드가 등재된 파일 안에 있음: `digest-budget.test.ts`(7),
  `finish-queue.test.ts`(15), `finish-drain.test.ts`(7). 셋 다 등재 확인 후 총 수 증가로 검증 — 충족
- 실브라우저에서 나갔다 와도 결과가 남는 것 확인 — 충족(아래)

실측 (실브라우저 + 운영 DB)

- 끝내기 등록 259ms, 응답에 `running/stage/jobId` — 예전엔 같은 버튼이 295초 뒤 절단됐다
- 도는 동안 버튼 disabled, 다른 화면 갔다 돌아와도 같은 `jobId` 와 `running: true` 유지
- 브라우저가 집어간 잡이 화면 이탈로 중단됨 → 임대 만료 뒤 크론이 이어받아 43초에 세 단계 완주
  (`retry_count` 1 → 2, `claimed/advanced/finished` = 1/3/1)
- Gemini 429 로 정리·5축 실패했으나 **사유가 `steps` 에 남았다** — 9/14 에는 같은 실패가
  `crm_ai_run` 에도 `system_events` 에도 없었다
- 잡 표 계약 여섯 가지를 트랜잭션 안에서 확인 후 롤백(잔여 행 0): 미완 잡 하나·선점·임대 회수·
  상한 못 박기·끝난 잡 뒤 재등록·상한 초과 제외

전체 diff 검토

- 변경 파일은 전부 항목 범위 안. 다른 세션 파일은 스테이징에서 제외(`--no-commit` 후 경로 지정)
- 비밀 0건, 한자 0건 (diff 전수 검색)

발견 사항

- **기존 가드가 버그를 고정하고 있었다.** `digest-progress.test.ts` 가 예산을 무시하는 옛 모양을
  그대로 단정해, 295초 사고가 나는 동안 초록이었다. 빠져 있던 계약(단계 예산 < 회차 예산 < 라우트 상한)을 세웠다
- **모든 단계가 실패해도 잡은 `DONE`** 이라 화면이 성공처럼 그릴 뻔했다. `failedSteps` 를 따로 꺼내 머리말을 가름
- 테스트 등재를 붙여 쓰면 셸이 이름 하나로 넘긴다 — `test-registry` 가드가 잡았다
- 부수 사고: 종합 감사로 돌린 `pnpm build` 가 dev 서버 `.next` 를 깨 전 화면 500 이 됐다.
  `.next` 삭제 후 재시작으로 복구. **dev 가 떠 있으면 build 를 돌리지 않는다**

남는 것 (이 판 밖)

- Gemini 한도가 소진돼 정리·5축의 **성공 경로**는 이 판에서 실측 못 했다. 한도가 풀리면 재확인 필요
- 「미팅 끝내기」 외의 정리 경로(회의노트 전용 `/digest`)는 아직 동기다
