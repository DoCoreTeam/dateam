...
실행 2026-09-21, 시작 커밋 659a2866 → 8b30b78a (내 커밋 13개, v0.10.300~v0.10.318)

### 검사 넷
- `pnpm tsc --noEmit` 오류 0
- `pnpm lint` 오류 0 (기존 react-hooks 경고만 남음, 내 파일 아님)
- `pnpm test` 6512/6512 통과, 실패 0 (플랜 시작 전 6488 → 새 가드 24개 늘어남)
- `NEXT_DIST_DIR=.next-p0040 pnpm build` 종료 코드 0 (dev 쪽 .next 를 안 덮었다)

### 완료 정의 대조
- 파일로 만든 견적서가 구성 줄과 금액 없는 행을 담는다 — 골든 가드가 항목 7·구성 13·금액 없는 줄 1 을 센다
- 건마다 자기 쪽을 갖고 대조가 그 자리를 연다 — 쪽은 마이그 273 칼럼에, 조각은 첨부로
- 상한과 켜고 끄기가 설정 화면에 뜬다 — 「견적서 파일 읽기」 카드 다섯 칸, 코드의 숫자는 선언 한 곳뿐(가드가 본다)
- 선언만 되고 안 쓰이는 값 0 — components·sourcePage·groupLabel·pageStart·sourceSnapshotId 전부 화면에서 쓰인다
  (groupLabel 이 한동안 안 쓰여 I13 을 추가해 메웠다)
- 사용자 노출 문구는 lib/terms 상수 (설정 정의의 라벨만 기존 관례대로 setting.ts 안에 둠)

### 보안 재측정 (7절 기계가 세는 것)
`docs/policy/security-count.sql` 운영 DB 실행, 다섯 줄 전부 0
- rls_off_tables 0 · anon_write_tables 0 · public_using_true_policies 0
- unpinned_secdef_functions 0 · anon_readable_secdef_views 0
마이그 273 은 새 표가 아니라 기존 crm_quote 에 칼럼 셋을 더한 것이고, 그 표의 RLS 는 켜져 있다(실측 relrowsecurity=t)

### 전체 diff
- 내 커밋이 건드린 파일 49개(버전 파일·플랜 제외), 전부 플랜 범위 안
- 비밀 0, 하드코딩된 상한 0, 내 미커밋 변경 0
- 같은 작업 트리의 다른 세션 커밋(v0.10.301~303, 305, 307, 311~312)은 내 커밋에 안 실렸다

### 항목 대 결과
I01~I13 열셋 전부 통과, 각 항목의 범위 파일이 실제로 존재하거나 변경됨

### 못 한 것
- **엑셀·한글 원본은 여전히 대조 화면에 못 그린다.** 쪽이라는 것이 없어 오릴 자리를 정할 수 없고,
  지금은 예전처럼 「이 형식은 화면 안에 못 그려요」와 내려받기로 떨어진다.
  읽은 표를 그림으로 다시 그리는 일이 필요하고, 그건 이 플랜의 항목이 아니다.
- **실브라우저 확인은 안 했다.** 시험·타입·빌드는 전부 초록이지만 pdfjs 로 쪽을 그리는 길은
  브라우저에서만 돈다. 그 길은 사용자가 파일을 올려 봐야 실제로 확인된다.
