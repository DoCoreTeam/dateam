...
### 1 전체 검사 (2026-09-20)
- `pnpm tsc --noEmit` — 내 범위 0건. 트리 전체로는 옆 세션의 미커밋 `app/admin/ai-usage/page.tsx` 1건이 뜬다
- `pnpm lint` — 오류 0건 (경고 2건은 내가 안 건드린 파일의 기존 것)
- `pnpm test` — 6247/6247 통과. 이 판에서 시험 190여 개가 늘었다
- `pnpm build` — `NEXT_DIST_DIR=.next-audit` 격리로 실행, 종료 코드 0. 끝내고 산출물과 tsconfig 원상 복구
- `pnpm design:check` — 통과 (진행 중 두 번 걸렸고 둘 다 그 자리에서 고쳤다)

### 2 완료 정의 대조
- 검사 넷 통과: 위와 같음
- 키 둘 이상일 때 첫 키가 429 면 다음 키로 넘어감: 실행으로 잼 (gemini-call·key-rotation·stt·host-caller 네 곳)
- 키 한 개일 때 동작이 지금과 같음: 호출 수를 세는 시험으로 네 곳 모두 확인
- 원문 API 키가 어떤 응답에도 안 담김: 가드가 네 방향으로 셈 (표를 여는 자리·브라우저로 가는 자리·가림값 만드는 자리·원장 key_ref)
- 사용자 노출 문자열이 전부 `@/lib/terms`: I11 에서 맞춤 (그 전까지는 안 맞았고, 그래서 항목을 하나 더했다)
- 설정값은 env 추가 없이 DB 저장과 UI 관리: 새 env 0개, 키는 `ai_provider_keys` 표와 관리자 화면

### 3 변경 범위
- 내 커밋 18 개, 건드린 파일 40 개. 전부 이 플랜의 항목 범위 안이고 범위 밖 수정은 셋뿐이며 사유를 그 항목 기록에 적었다
  (`lib/gpu/ai-budget.test.ts` 내 회귀 · 정책 3파일의 역사 인용 복구 · `apps/web/package.json` 등재)
- 비밀 스캔: 실제 키 0건. 잡힌 셋은 전부 가림값을 시험하려고 지어낸 문자열이다
- pre-commit 보안 가드가 커밋마다 「비밀 0 · RLS 누락 0」을 찍었다
...
- 마이그레이션 264(표 신설)와 266(칸 추가) 적용 완료. 264 는 같은 판에서 RLS 를 켜고 정책을 0개로 두었고,
  266 은 표를 만들지 않아 `ai_llm_calls` 의 RLS 가 그대로다 (실측 `relrowsecurity = t`, anon 쓰기 0건)

### 4 항목 대 결과
- 신규 파일 12 개가 전부 추적되고, 시험 다섯은 `package.json` 등재 목록에서 실제로 돈다
- 등재만 하고 안 도는 상태를 막으려고 커밋 전마다 「등재됐는데 추적 안 되는 파일」을 셌다
  (실제로 두 번 걸렸다 — 옆 세션의 미추적 파일이 내 커밋에 실릴 뻔했고, 한 번은 실려서 다음 커밋으로 되돌렸다)

### 5 발견 (지시 밖)
- 옆 세션의 버전 올리기가 파일의 첫 `vX.Y.Z` 를 갈아 정책 3파일의 역사 인용을 깨뜨린다. 내 것도 같은 버그였고 고쳤다.
  깨진 세 자리를 되돌려 놨으나 그쪽 도구는 그대로다
- `app/admin/ai-usage/AiUsageDashboard.tsx`(옆 세션 미커밋)에 전각 대시 1건과 미정의 토큰 3건이 있다. 내 범위가 아니라 기록만 한다

### 6 못 한 것
- 실브라우저 QA(E-1~E-5). 관리자 세션이 있어야 하고, 시험이 운영 AI 키 표를 더하고 지우는 일이라
  되돌릴 수 없는 데이터 변경에 해당한다. 사용자가 직접 밟을 자리를 보고에 적었다
