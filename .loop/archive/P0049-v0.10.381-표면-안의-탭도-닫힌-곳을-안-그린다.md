...
## 종합 감사
2026-09-22, 시작 커밋 7bb8416c 기준

### 1 검사 넷
- `pnpm tsc --noEmit` 통과 · `pnpm lint` 오류 0 · `pnpm test` **6659/6659** (판 시작 6654 → 가드 5개 늘었다) · `NEXT_DIST_DIR=.next-p0049 pnpm build` 성공 · `pnpm design:check` 통과

### 2 완료 정의 한 줄씩
- 검사 넷 통과 — 위와 같다
- **부여 0건이면 이 판 앞뒤가 같다** — 관리자·일반 사용자 둘 다 업무 탭 다섯, 구 영업 탭 넷 + CRM 링크, 홈 바로가기 셋, 계정 메뉴 넷 그대로
- **닫으면 함께 사라진다** — 주간보고(전체 메뉴·업무 탭바·주소) · 캘린더(사이드바·전체 메뉴·주소) · KPI(홈) · 보안과 개발자센터(계정 메뉴). 표면마다 그려지는 자리가 달라 한 표면으로 넷을 동시에 보일 수 없어 나눠 쟀다
- **가드가 잡는다** — 판정을 안 보는 내비게이터를 신설해 실패를 확인하고 지웠다. 홈에서 판정 호출을 들어내도 실패한다

### 3 전체 diff
`git diff 7bb8416c..HEAD --stat` 19파일 468추가 35삭제. 비밀 0건, 한자 0건

### 3a 보안 재측정 (7절, 운영 DB)
rls_off_tables 0 · anon_write_tables 0 · public_using_true_policies 0 · unpinned_secdef_functions 0 · anon_readable_secdef_views 0 — **전부 0**

### 4 항목 대 결과
신규 둘(`lib/access/open-context.tsx` · `lib/ui/cross-surface-nav.test.ts`)이 있고, 고친 내비게이터 다섯이 전부 판정을 부른다

### 5 발견 사항
- **가드를 먼저 세운 값** — 손으로 훑어 셋인 줄 알았는데 다섯이었다. 계정 메뉴(SidebarProfile)와 AI 넘김 모달(WorkflowHandoffModal)은 가드가 찾아 줬다
- **조용히 통과하던 가드** — 경로에 한글이 있어 `URL.pathname` 이 퍼센트 인코딩을 주는 바람에 파일을 0개 찾고도 통과했다. 「세는 일 자체가 되고 있다」 단정이 잡았다
- **이름을 찾는 가드의 함정** — 홈에서 import 만 지웠더니 `canOpen` 문자열이 남아 그대로 통과했다. 호출 블록을 통째로 지워야 실패했다
- **오늘 못 재는 것 둘** — 구 영업 탭바의 「넷 다 닫히면 안 그린다」와 AI 넘김 모달의 거르기. 둘 다 그 화면에 닿으려면 최소 하나가 열려 있거나 관리자여야 해서 도달할 수 없다. 분기는 넣고 이유를 주석에 적었다
