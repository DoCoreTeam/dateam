...
- pnpm lint: exit 0 (남은 경고는 전부 기존 useCallback 의존성)
- pnpm test: 6378건 전부 통과, 실패 0건 (1차 6376/2실패 → 보완 항목 I10 뒤 6378/0)
- NEXT_DIST_DIR=.next-p0037 pnpm build: exit 0, 정적 293쪽 생성, 끝난 뒤 .next-p0037 삭제와 tsconfig 원복

### 2 완료 정의 대조
- 할인 0인 견적서에서 「할인」이 화면·인쇄·엑셀 어디에도 안 나온다 → hasDiscount 한 판정을 셋이 함께 보고, 가드 quote-discount-visibility 가 잠금
- 설정의 기본 거래 조건을 바꿔도 이미 만든 견적서의 조건 줄이 그대로다 → termsSnapshot(마이그 270), 기존 35건 전부 굳음
- 설정의 상호·대표이사·로고를 바꿔도 견적서의 공급자 칸과 로고가 그대로다 → supplierSnapshot·logoAssetHash(마이그 271), 기존 35건 전부 굳음
- 파일로 가져오기로 만든 견적에 원본 파일이 붙는다 → target=QUOTE 로 견적마다 붙고 견적 상세 첨부 절에서 보인다
- 사용자 노출 문자열은 전부 lib/terms 상수 → IMPORT_KEEP_FILE 세 줄을 고쳐 재사용, 신규 하드코딩 0건
- 설정값은 env 추가 없이 DB 저장 → 이번 판에 새 설정값 없음

### 3 전체 diff
- 이 플랜의 커밋 열 개가 건드린 파일은 전부 항목 범위 안(버전 파일 다섯과 PLAN.md 제외)
- 비밀 검사: 추가된 줄에서 키·토큰·비밀번호·접속 문자열 0건
- 한자 검사: 추가된 줄 0건

### 3a 보안 재측정 (LOOP.md 7절 「기계가 세는 것」)
운영 DB 에서 다섯 줄을 그대로 실행
...
새 표 crm_quote_asset 은 같은 판에서 RLS 를 켜고 anon·authenticated 권한을 회수했다
(information_schema 확인 0건, pg_class relrowsecurity = t)

### 4 항목 대 결과 대조
- I01~I10 열 항목의 범위 파일이 전부 존재하거나 실제로 바뀌었음을 커밋 열 개로 확인
- 마이그 270·271 은 운영 DB 에 적용되어 supabase_migrations 에 등록됨

### 발견 사항 (보완 완료)
- 종합 감사 1차에서 2건 실패: 새 표 CrmQuoteAsset 이 workspace-guard 분류에 없어 direct 로 잡혔고, 원본 남기기 기본값 변경이 quote-layout 가드와 어긋났다 → I10 으로 둘 다 맞추고 재실행 0건
- 부수 수정: updateQuote 가 termIds 를 받아 놓고 저장하지 않아 견적 편집에서 거래 조건을 고쳐도 아무 일이 없던 결함
- 부수 수정: 견적 상세에 첨부 절이 없어 QUOTE 첨부를 볼 자리가 없던 것

### 한계 (숨기지 않고 적음)
- 마이그 271 의 백필은 **오늘의 공급자 값**을 기존 견적에 넣었다. 그 견적이 만들어진 날의 값은 설정이 덮어쓰기라 어디에도 남아 있지 않다. 여기서부터는 안 바뀐다
- 실브라우저 검증은 안 했다. 시험·가드·빌드·운영 DB 실측으로만 확인했다
