...
실행 (2026-09-17, apps/web)

- pnpm tsc --noEmit → 0건
- pnpm test → 5,748 통과 / 2 실패, **둘 다 내 범위 밖**
  - app/admin/members/[id]/EmploymentCard.tsx (재직 상태 세션)
  - app/admin/org-chart/DeleteNodeModal.tsx (조직 삭제 세션)
- pnpm build → 통과 (NEXT_DIST_DIR 로 격리, 공용 dev 서버 무영향)
  - 처음에는 실패했음. 원인은 옆 세션의 AI 게이트웨이 이관(v0.10.95~96)이
    client 컴포넌트(IntakeModal)가 쓰던 card-read.ts 에 서버 전용 경로를 끌어들인 것.
    /crm/deals, /crm/companies 가 「Build Error」 화면만 뜨는 상태였고 내 결과를 볼 수도 없어
    한도 상수만 card-limits.ts 로 갈라 풀었음 (v0.10.117, 플랜 밖 단독 커밋)

완료 정의 대조

- 원문 수정 중 [저장][취소] 보임 — 실브라우저 확인
- 취소가 「수정」 누른 시점으로 되돌림 — 코드·가드, 무변경 취소는 실브라우저 확인
- 회의노트에서 회사·딜 붙이기·바꾸기 — 실브라우저에서 실제로 붙임(용인시티포인트 유지보수)
- 뽑은 할 일이 딜에도 섬 — 실브라우저 전 구간: 뽑기 13건 → 반영 → 「개인 일일업무와 딜 양쪽에 6건 세웠어요」
  → crm_task 6건 전부 dealId·sourceMeetingId 적재 → 딜 상세 「다음 할 일」에 6건 표시
- 검증 데이터 전량 원복 (crm_task 6 · daily_logs 6 · calendar_events 1 삭제, crm_meeting.dealId 다시 null)

전체 diff

- git diff 8df2b4da^..HEAD --stat 기준, 내 커밋 7개(v0.10.97·98·100·105·107·108 + 플랜 밖 117·118)
- 범위 밖 변경 없음, 비밀 없음, 화면 한글은 lib/terms 상수 경유

발견 (지시 밖, 고치지 않음)

- 위 두 테스트 실패는 다른 세션의 진행 중 작업
- 정책 3파일의 지난 판 번호를 내 치환이 건드린 것을 발견해 되돌림(v0.10.118)
