...
### I13 읽은 묶음이 견적의 묶음이 된다
상태: 통과
모드: 경량
범위: apps/web/components/ui/crm/quote-review.tsx, apps/web/components/ui/crm/QuoteFromFileModal.tsx, apps/web/lib/crm/ui/quote-source-surface.test.ts
감사 기준:
- 원본이 묶어 부른 말(groupLabel)이 검수 모양에 실려 오는 단정이 있다
- 그 말로 견적 묶음(sections)이 만들어지고 항목이 제 묶음에 들어가는 단정이 있다
- 묶음 이름이 하나도 없으면 묶음을 안 만드는 단정이 있다 (빈 묶음이 생기면 소계가 헛돈다)
- 보안: 화면과 기존 견적 창구만 쓴다, 새 데이터·창구·외부 입력 없음
의존: I05
...
- v0.1.2 (2026-09-21) I13 추가 (audit:I12) 읽기 모양에 넣은 묶음 이름을 아무 데서도 안 써 「선언만 되고 소비 코드 0」 이 되던 자리
...
- v0.1.2 (2026-09-20) I13 추가: 읽기 모양에 넣은 묶음 이름(groupLabel)을 아무 화면도 안 써서 선언만 되고 소비 코드 0 인 자리가 됐다. 완료 정의가 그것을 금지한다 (audit:I12)
