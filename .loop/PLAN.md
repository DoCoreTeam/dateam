...
상태: 통과
모드: 경량
범위: LOOP.md, scripts/loop.mjs, apps/web/lib/policy/version-rule.test.ts
감사 기준:
- 항목 커밋 형식에서 -Ixx 꼬리가 사라지고 vX.Y.Z 한 가지만 남음 (사용자 지시 iv_0052 계열)
- 커밋마다 패치가 1 오르고 여섯 파일이 함께 오름 (앞 버전을 그대로 복사하면 그 커밋은 사용자에게 안 보인다)
- loop.mjs 의 자동 커밋 메시지가 그 형식을 만들고, 버전을 스스로 올림
- 가드가 새 커밋에서 -Ixx 형식을 거부하는 단정 (기준선 이후만)
- 가드를 일부러 깨뜨려 실패를 확인한 근거를 pass notes 에 기록
- node --test 로 version-rule.test.ts 통과
의존: 없음
...
- v0.3.0 (2026-09-14) 사용자 지시 - 항목 커밋에 -Ixx 를 붙이지 말고 세 번째 자리(패치)를 올리는 방식으로. I15 추가 (iv_0052)
