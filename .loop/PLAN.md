...
### I06a 공유 파서가 엑셀을 두 번 읽고 표를 못 본다
상태: 통과
모드: 경량
범위:
...
감사 기준:
- 실제 xlsx 견적서를 넣으면 셀 글자가 한 번만 나온다 (지금은 「H100 80GB SXM H100 80GB SXM」)
- 시트가 표로 읽힌다: tableCount 가 0 이 아니고, 행마다 「A | B | C」 로 펴진다
- 셀 metadata 의 row·col 을 쓴다: 빈 칸이 중간에 있어도 열이 밀리지 않는다 (밀리면 수량 자리에 단가가 온다)
- 기존 office.test.ts 20건이 그대로 통과한다 (RFP 인입이 같은 파서를 쓴다)
- pnpm test 통과
의존: 없음
...
명령 (2026-09-19)
- pnpm typecheck: 통과 (오류 0)
- pnpm lint: 통과 (exit 0, 새 경고 없음)
- pnpm test: 5856/5856 통과 (플랜 시작 시점 5759 → +97)
- pnpm design:check: 통과 (hex 0, 기준 초과 0)
- next build (NEXT_DIST_DIR=.next-quotefill): 통과, /api/crm/quotes/draft-file 가 빌드 목록에 뜸. 빌드가 tsconfig 에 자동으로 넣은 include 줄은 되돌림

실화면 (Playwright · dev 3000 · 로그인 세션)
- 딜 상세 → 새 견적: 단추 넷이 같은 줄에 서고 서로 40px 안에 붙으며 오른쪽에 모임 (예전엔 space-between 으로 흩어짐). 스크린샷 artifacts/quote-fill/01-toolbar.png
- 엑셀 견적서 올리기: 3건을 읽고 검수 목록이 뜸. 합계 대조 「읽은 금액 143,000,000원 / 문서 합계 143,000,000원 · 문서에 적힌 합계와 맞아요」(초록). 줄마다 원문 조각이 함께 보임. 03-review.png
- 「체크한 항목 넣기」를 누르면 품목·규격·수량·단위·단가가 폼에 그대로 들어감. 04-applied.png
- 창구 직접 호출: xlsx 200(3건·못 알아봄 0)·docx 200(2건, 유지보수를 PERIOD 로 판정)·텍스트 PDF 는 Gemini 429 라 미검증이나 화면이 「AI 사용량 한도를 초과했습니다」를 정확히 띄움

완료 정의 대조
- 네 명령 전부 통과: 충족
- 새 테스트가 실제로 도는가: 등재 전후 총 수가 5759 → 5856 으로 늘어 확인 (quote-source-text 14 · quote-from-doc 20 · quote-from-file 23 · quote-reconcile 26 · office 10 · quote-layout 5 개 증가분 포함)
- 화면 문자열이 lib/terms 를 거치는가: 새 문자열 30여 개 전부 quote.ts 경유, 가드가 리터럴 복귀를 막음
- 사람이 확인하기 전에는 폼에 안 들어가는가: 서비스에 DB 쓰기 0 · 화면은 체크한 줄만 반영, 넷을 일부러 깨서 확인
- 합계가 다르면 말하는가: checkTotal 이 차액과 방향을 문장으로 돌려주고 화면이 data-verdict 로 그림
- env 추가 없음: 충족

발견 사항 (실행해 보고서야 드러난 것)
- 공유 오피스 파서가 엑셀 셀을 두 번 읽고 있었다 (「H100 H100」·「2 2」). 단위 테스트·타입 검사·리뷰가 전부 초록이었다 → I06a
- 같은 파서가 시트를 표로 안 읽어 셀 경계가 사라졌다 → I06a
- 형식 판별을 파서에게 맡겨 Next 런타임에서만 parse_failed 로 죽었다. 노드로 직접 돌리면 읽혀서 실행 검사로도 못 잡는다 → I06b (코드 모양을 정적으로 잠금)
- 셋 다 RFP 인입이 쓰는 같은 경로라 그쪽도 함께 고쳐졌다

남은 것
- 텍스트 PDF 경로는 Gemini 무료 티어 한도 때문에 AI 단계를 실측하지 못함. 파싱 단계는 실측으로 확인했고(줄 추출 정상), 한도가 풀리면 같은 파일로 재확인할 것
- PDF 는 표 구조가 없어 tableCount 가 0 이고 화면이 「표를 찾지 못해 글줄만 읽었어요」를 띄운다. 사실이므로 그대로 둠
