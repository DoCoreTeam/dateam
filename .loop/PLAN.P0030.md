...
### I19 상한 이름이 창구 이름과 어긋나지 않게 한다
상태: 통과
모드: 중량
범위: apps/web/lib/ai/budget.ts, apps/web/lib/ai/budget.test.ts, apps/web/lib/ai/budget-gate.ts, apps/web/lib/ai/usage-query.ts, apps/web/lib/ai/usage-query.test.ts, apps/web/lib/policy/ai-budget-gate.test.ts, supabase/migrations/268_ai_call_budget_coverage.sql (신규)
감사 기준:
- 창구 이름이 상한 이름과 정확히 안 맞아도 **앞자리를 타고 올라가** 상한을 찾음 (crm/quick_create 가 crm 을 물려받음, 단위 테스트)
- 맞는 줄이 여럿이면 **가장 좁은 것**이 이김 (단위 테스트)
- 받아 주는 줄(`*`)이 있어 어떤 창구도 무제한이 아님 (단위 테스트 + 운영 DB 실측)
- 등재부의 창구 전부가 상한에 닿음, 안 닿는 창구가 생기면 가드가 실패 (일부러 하나 빼서 확인)
- 화면이 물려받은 상한을 「상한 없음」으로 안 그리고 어디서 온 값인지 보임
- 보안 S1: 줄만 넣으므로 잠금이 안 바뀜을 운영 DB 에서 확인
의존: I09, I14
범위 메모: 사용자 지시(ins_0042)로 시작. 착수 전 실측에서 어긋남이 crm 하나가 아니라 **창구 41 중 32 가 상한이 아예 없는 것**으로 드러났다 — 게이트가 모르면 통과라 그 32 는 지금 무제한이다. 원장이 나흘치뿐이라 34 곳은 관측이 없어 값을 지어낼 수 없으므로, 받아 주는 줄 하나로 무제한을 없애고 관측이 쌓이면 화면에서 조정한다


### I17 실브라우저에서 트렌드 화면과 사용량 화면을 확인한다
상태: 대기
모드: 경량
범위: 코드 변경 없음 (확인만)
감사 기준:
- 트렌드 화면이 「새로 물을 것 N건」을 보이고 0건이면 버튼이 안 눌림 (스크린샷)
- 관리자 사용량 화면의 오늘 숫자가 psql 집계와 같음 (스크린샷)
- 상한 바꾸기가 관리자에게만 열리고 비관리자에게는 403 (실제로 눌러 확인)
의존: 없음
범위 메모: 이 판에 dev 서버도 관리자 세션도 없어서 못 했다. 배포된 판이나 관리자 로그인이 있으면 바로 된다

### I18 적용 하루 뒤 원장으로 감축을 잰다
상태: 대기
모드: 경량
범위: 코드 변경 없음 (확인만)
감사 기준:
- 배포 하루 뒤 ai_llm_calls 에서 하루 호출이 40건 이하인지 실측하고 숫자를 적음
- 거절이 실제로 남는지 확인 (지금은 0건, 게이트가 배포 전이라 안 걸린다)
- 상한 표의 기능 이름이 원장의 surface 와 다 맞는지 재확인 — crm 이 crm/quick_create 와 안 맞아 그 상한이 한 번도 안 걸렸다
의존: 없음
범위 메모: 배포는 사용자가 한다. 착수 직전 실측 기준값은 오늘분 8,729건(성공 927 / 실패 7,802 / 거절 0)
...
### 1 명령 넷
    ...
    pnpm lint                              exit 0
    pnpm test                              6,274 / 6,274 통과 (실패 0)
    NEXT_DIST_DIR=.next-final pnpm build   exit 0

빌드는 격리 판으로 돌렸고 끝나고 tsconfig 와 next-env.d.ts 에 남은 줄을 되돌렸다
...
|---|---|---|
| 명령 넷 통과 | 충족 | 위 |
| 콘텐츠 1건 인입이 발견 배치를 부르지 않음 | 충족 | trigger-scope 가드 |
| 같은 대조쌍을 두 번 물으면 두 번째는 벤더에 안 나감 | 충족 | contrast-key + discover-stop 가드 |
| 자동으로 부르는 AI 가 전부 「이미 봤음」을 거름 | 충족 | new-only 가드 |
| 하루 상한에 닿으면 거절되고 사유와 풀리는 시각이 응답에 있음 | 충족 | ai-budget-gate 12단정 |
| 관리자 사용량 화면이 ai_llm_calls 를 읽음 | 충족 | usage-query 8단정 |
| 개발 판에서 운영 공급자 키를 쓰지 않음 | 충족 | ai-key-source + provider-key-source |
| 실브라우저에서 트렌드 화면 확인 | **미충족** | 이 판에 dev 서버도 관리자 세션도 없다 (I17) |
| 적용 하루 뒤 원장 실측 하루 40건 이하 | **미충족** | 배포가 있어야 잰다, 배포는 사용자가 한다 (I18) |

위 가드 여덟을 한 번에 돌려 62 단정 전부 통과

### 3 전체 diff

    git diff 590a62dd..HEAD --stat   245 파일, +12,510 / -503

옆 세션(P0031 키 교체)과 한 작업 트리를 나눠 썼으므로 이 수치에 그쪽 변경이 섞여 있다
내 커밋은 이 구간에서 22개이고 커밋마다 내 경로만 스테이지했다

비밀 0건 — 정규식에 걸린 넷은 전부 옆 세션 시험의 가짜 키(`gsk_1234...`, `AIzaSyD-1234...`)이고
`.test.ts` 안에 있다. 새 env 0건. 미커밋 변경 0건(untracked 한 개는 옆 세션의 옛 플랜 archive)

### 4 기계가 세는 다섯 줄 (운영 DB 실측)

    1 RLS 꺼진 public 표            0
    2 anon 이 쓰기 권한 가진 표      0
    3 TO public 이면서 USING(true)  0
    4 search_path 없는 SECDEF 함수  0
    5 anon 이 읽는 SECDEF 뷰        0

### 5 항목 대 결과

이번 플랜이 만든 파일 열 개가 전부 제자리에 있다 — budget-gate·actor·capability-chain·
output-limit·usage-query·deploy-env·provider-key-source·discover-prompt·gemini-embedding.test·
마이그 267. 마이그 267 은 운영에 적용했고 적용 뒤 잠금이 안 바뀐 것을 확인했다

### 6 남은 것 둘

둘 다 **배포가 있어야 재는 것**이고 배포는 사용자가 한다

- I17 실브라우저에서 트렌드 화면 확인
- I18 적용 하루 뒤 원장 실측으로 하루 40건 이하 확인

## 변경 이력
- v0.1.23 (2026-09-20) 종합 감사 결과를 기록하고 못 잰 둘을 항목으로 남김. 완료 정의 아홉 줄 중 일곱이 충족이고 둘은 배포가 있어야 재는 것이다 — 실브라우저 확인(I17)과 적용 하루 뒤 원장 실측(I18). 배포는 사용자가 하므로 조용히 넘기지 않고 대기 항목으로 남긴다 (audit:final)
- v0.2.0 (2026-09-20) 사용자 개입(ins_0042 상한 이름 안 맞는 거 고쳐줘)으로 I19 추가. 착수 전에 재어 보니 어긋남이 crm 하나가 아니라 창구 41 중 32 가 상한이 아예 없었다 — 게이트가 모르면 통과라 그 32 는 무제한이다. 이름을 하나 고치는 대신 앞자리를 타고 올라가는 규칙과 받아 주는 줄로 구조를 고친다 (iv_0042)
