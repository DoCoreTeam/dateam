...
- I14 통과 시점의 `pnpm test` 가 4602 중 2건 실패였고 그것을 통과 기록에 4601/4601 로 잘못 적었다
  - 실패한 두 건은 내 범위 밖: `lib/ui/glossary.test.ts` 의 「—」 금지 가드(`lib/terms/report.ts` 미커밋 수정)와 `lib/ui/card-standard.test.ts` 의 카드 여백 가드(`app/globals.css` 미커밋 수정)
  - 둘 다 다른 세션의 편집 중간 상태였고 재실행 시 4602/4602 통과로 돌아왔다. 코드는 손대지 않았다
- I15 통과 시점에도 같은 종류로 3건 실패 (전부 다른 세션의 `app/(ai)/ai` 이동 중간 상태)
  - `lib/ai-chat/nav/groups.test.ts` 옛 주소 하드코딩 · 진행 표시 ratchet · 화면 라벨 맵 ratchet
  - 내 커밋에는 `app/` 파일이 0개다. 코드는 손대지 않았다
- 배운 것: 항목 통과 전에 `pnpm test` 결과를 **먼저 확인하고** 통과 명령을 내려야 한다. 한 줄로 이어 붙이면 실패를 못 보고 지나간다

## 운영 메모 (2026-09-09)
- 다른 세션이 `loop plan new` 로 P0002 를 만들며 `.loop/PLAN.md` 를 덮었고, loop-kit 이 이 플랜을 `.loop/archive/` 로 보관했다
...
- I41 의 결과 기록 폼과 정정공고 비교를 케이스 상세(`/rfp/[id]`)에 꽂았다 — 범위를 미루지 않았다
  ...
  - 실호출로 확인: 저장 200(decision=go, submitted=true, result=lost, our_rank=2),
    안 냈는데 순위를 적으면 400 rank_without_submit
  - 부수: `CrossVerifyDialog` 의 `fields=` 프롭이 레거시 3슬롯 가드와 이름이 겹쳐 `candidates=` 로 바꿈
...
- I23 실호출에서 내 회귀 1건을 잡아 고쳤다(M-9 ①): `@rhwp/core` 의 `rhwp_bg.wasm` 을 webpack 이 번들하려다
  ...
  - 가드 `lib/ui/deploy-fragile.test.ts` 의 BINARY_BACKED 에 사유와 함께 추가(그 가드가 요구하는 절차다)
  - 격리 빌드(`NEXT_DIST_DIR=.next-rfp-check`)로 확인 후 산출물과 tsconfig 자동 변경을 되돌림
...
의존: K02, K03
- (2026-09-14, 다른 세션이 고침) 목표 버전을 v0.8.0 에서 v0.10.3 으로 옮김. v0.8.0 은 이미 지나간 버전이라(package.json 0.10.1) 완료 커밋이 package.json 을 올리지 못하고 이 플랜의 결과가 사용자 화면에 영영 안 보인다. LOOP.md 부록 버전 규칙을 고치면서 동시 플랜은 같은 minor 안에서 patch 를 나눠 갖게 바뀌었고 P0002 가 0.10.2, 이 플랜이 0.10.3 을 맡는다
