...
상태: 통과
모드: 중량
범위: apps/web/lib/policy/ai-key-pool.test.ts (신규), apps/web/package.json
감사 기준:
- 가드가 ai_provider_keys 를 key-store 밖에서 직접 읽는 자리 0건을 셈
- 가드가 화면과 응답에 원문 키를 싣는 자리 0건을 셈
- 가드가 키 교체를 자기 방식으로 다시 짠 자리 0건을 셈 (공용 부품 하나만 쓴다)
- 가드가 키를 다루는 모듈에 server-only 가 붙어 있는지 셈
...
- 키 이름을 안 주는 기존 호출은 그대로 동작함 (칸이 비어 있을 뿐 기록이 멈추지 않음)
- 보안: 마이그레이션이 표를 새로 만들지 않고 기존 ai_llm_calls 에 칸만 더함, 그 표의 RLS 상태가 전과 같음
- 보안: 가드가 key_ref 에 들어가는 값이 label 임을 셈 (apiKey 를 넣는 자리 0건)
- pnpm tsc --noEmit 통과
의존: I10
막힘 없음: 마이그레이션 번호 265 는 옆 세션이 가져가 266 으로 옮김. 칸 추가는 되돌릴 수 있음(drop column)

### I11 키 화면의 말을 용어집으로
상태: 통과
모드: 경량
범위: apps/web/lib/terms/ai-key.ts (신규), apps/web/lib/terms/index.ts, apps/web/lib/ai/key-store-core.ts, apps/web/app/admin/settings/AiProviderCard.tsx, apps/web/lib/ai/provider-keys.test.ts
감사 기준:
- 키 화면과 상태 문구가 전부 @/lib/terms 에서 옴 (화면과 규칙 모듈 둘 다)
- 금지어를 새로 들이지 않음 (추가·지우기·등록 대신 새 키·삭제·저장)
- 상태 말이 한 곳에만 있음: 같은 상태가 화면마다 다른 말이 되지 않게
- 용어집 가드(lib/ui/glossary.test.ts)와 설정 화면 가드(lib/ui/settings-parity.test.ts) 통과
- pnpm test 통과
의존: I09a
...
- v0.1.12 (2026-09-20) 완료 정의의 「사용자 노출 문자열은 전부 @/lib/terms」가 아직 안 맞아서 항목을 하나 더한다. I09a 에서 상태 문구를 규칙 모듈이 만들게 해 화면 하드코딩은 없앴지만, 그 말들이 용어집이 아니라 lib/ai 안에 있다. 새로 들인 말(키 이름·새 키·위로·아래로·켜기·끄기)도 화면에 직접 적혀 있다. 완료 정의를 못 맞춘 채로 끝내지 않는다 (audit:I11)
