...
> **✅ 2026-07-14 v0.7.300 업데이트 (Mac 세션):** 🔴 BLOCKER 전부 해소.
> - N-1: 마이그 150~153 라이브 적용 완료 → AI채팅 표 8개 + storage `ai-chat` 버킷 생성 확인.
> - N-4: 149 번호충돌 정리(`149_org_weekly_reports`→`155`) 후 적용 → `org_weekly_reports` 생성.
> - N-2/N-3: 이 Mac에선 migrate.sh 정상·DB비번 확인됨(Windows 전용 이슈라 무관).
> - 추가: 좌측 메뉴 'AI 채팅'(admin 전용) 배선(이전엔 진입로 자체가 없어 "화면에 안 뜸"의 직접 원인).
> **남은 것:** 배포(커밋→푸시)해야 라이브 반영. 🟡 GAP-* 완결도 항목은 별도.
