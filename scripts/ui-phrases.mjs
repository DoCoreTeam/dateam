...
// '되돌릴/삭제할 수 없습니다'는 **경고문**이지 빈 상태가 아니다.
// 단 '찾을 수 없습니다'(not found)는 빈 상태가 맞다.
//
// ⚠️ 두 번 틀렸던 자리다.
//   ① `(?<!되돌릴 )없습니다` 는 **한 번도 안 걸렀다** — 능력부정은 '되돌릴 **수** 없습니다'라
//      `없습니다` 바로 앞이 언제나 `수 ` 이기 때문이다. 그래서 경고문을 쓴 화면이 전부
//      '빈 상태 자작'으로 잡혀 **다른 세션의 커밋까지 막았다**(v0.7.493 실측).
//   ② 그 다음 판은 능력 동사를 손으로 나열했는데, 목록에 없는 동사가 나올 때마다 또 막혔다
//      (2026-09-20 실측 — 「들어올 수 없습니다」).
//   ③ 뒤보기 정규식으로 뒤집어 보니 `\S+` 가 토큰 중간('찾을'의 '을')부터 걸려 또 틀렸다.
//
// 그래서 정규식을 더 비틀지 않고 **말을 먼저 걷어낸 뒤에** 센다.
// 「무엇 + 수 + 없다」 한 덩어리를 지우되, 빈 상태가 맞는 몇 개는 남긴다.
const EMPTY_EVEN_WITH_SU = /(?:찾을|발견할|조회할) ?수 ?(?:없습니다|없어요)/
const ABILITY_NEGATION = /\S+ ?수 ?(?:없습니다|없어요)/g

/** 「…할 수 없습니다」를 지운다. 목록에 있는 몇 개(찾을·발견할·조회할)는 남긴다. */
export function stripAbilityNegation(line) {
  return line.replace(ABILITY_NEGATION, (m) => (EMPTY_EVEN_WITH_SU.test(m) ? m : ' '))
}

export const EMPTY = '(?:없습니다|없어요)|비어 ?있습니다'
...
export function isSelfMadeStateText(line, phrase) {
  // 빈 상태를 셀 때만 능력부정을 먼저 걷어낸다. 로딩·오류 문구와는 글자가 겹치지 않는다.
  const text = phrase === EMPTY ? stripAbilityNegation(line) : line
  return jsxText(phrase).test(text) || bareText(phrase).test(text)
}
