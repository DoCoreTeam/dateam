#!/usr/bin/env node
// loop-kit v0.1.0  scripts/loop.mjs
...
catch { console.error('[loop-kit] node:sqlite 사용 불가, Node 22.13 이상 필요'); process.exit(1); }
...
const die = (s) => { console.error('[loop-kit] ' + s); process.exit(1); };
...
function resumeText() {
  ...
  const nx = nextItem(p);
  if (nx && statusKey(nx.status) === '진행중') lines.push(`[loop-kit] 이전 컨텍스트에서 진행중이던 항목 ${nx.id}, 구현 상태 불명이므로 재감사 대상`);
  else if (cur) lines.push(`[loop-kit] 기록상 진행 항목 ${cur} (플랜 상태와 불일치 시 플랜 우선)`);
  if (nx) { lines.push('[loop-kit] 다음 대상 항목'); lines.push(nx.block.slice(0, 1800)); }
  else if (planActive(p)) lines.push('[loop-kit] 대기 항목 없음, 종합 감사 단계');
  ...
  for (const iv of ivs.reverse()) lines.push(`[loop-kit] 최근 개입 ${iv.id}${iv.item_id ? ' (' + iv.item_id + ' 진행 중)' : ''}: ${iv.content.replace(/\s+/g, ' ').slice(0, 160)}`);
  const cmds = ['cmd_typecheck', 'cmd_lint', 'cmd_test', 'cmd_build'].map((k) => `${k.slice(4)}=${getSetting(k)}`).join(', ');
  lines.push(`[loop-kit] 감사 명령 ${cmds}`);
  lines.push('[loop-kit] 재개 규칙과 감사 기준은 LOOP.md 기준');
  return lines.join('\n');
}
...
cmds.init = (a) => {
  ...
  out(`[loop-kit ${KIT_VERSION}] 초기화 완료: ${path.relative(ROOT, DB_PATH)}, ${hooksFile}${cursorFile ? ', ' + cursorFile : ''}, 레지스트리 ${REGISTRY_PATH}`);
  out(resumeText());
};
...
cmds.hook = (a) => {
  ...
  if (!fs.existsSync(DB_PATH)) { if (event === 'session') out(`[loop-kit ${KIT_VERSION}] 미초기화, node scripts/loop.mjs init 필요`); return; }
  if (event === 'prompt') {
    ...
    if (planActive(p)) {
      ...
      if (isCursor) out('{"continue":true}'); else out(`[loop-kit] 개입 기록 ${id} (플랜 ${p.header.id} ${p.header.version}${currentItem() ? ', 진행 항목 ' + currentItem() : ''})`);
    } else {
      ...
      if (isCursor) out('{"continue":true}'); else out(`[loop-kit] 지시 기록 ${id}, 활성 플랜 없음`);
    }
    return;
  }
  if (event === 'edit') {
...
cmds.plan = (a) => {
  ...
  if (sub === 'new') {
    ...
    out(`[loop-kit] 플랜 ${planId} 생성 ${path.relative(ROOT, PLAN_PATH)}, 항목 작성 후 plan check 실행`);
    return;
  }
  const p = readPlan();
  if (sub === 'check') {
  ...
  if (sub === 'confirm') {
    ...
    out(`[loop-kit] 플랜 ${p.header.id} ${p.header.version} 진행중, 첫 항목은 start ${nextItem(readPlan()).id}`);
    return;
  }
  if (sub === 'revise') {
    ...
    if (ref && ref.startsWith('audit:')) {
      ...
      if (c >= num('max_plan_revisions_per_item')) out(`[loop-kit] 주의: ${itemId} 감사 기인 플랜 갱신 ${c}회, 한계 ${num('max_plan_revisions_per_item')}회 도달, 사용자 판단 요청 권고`);
    }
    out(`[loop-kit] 플랜 ${p.header.id} ${p.header.version} -> ${v} (${trigger})`);
    return;
  }
  if (sub === 'snapshot') { const r = snapshot(a.trigger || 'manual', a.ref || null); out(`[loop-kit] 스냅샷 ${r.skipped ? '변경 없음' : '저장'} ${r.version}`); return; }
  if (sub === 'next') { const nx = nextItem(p); out(nx ? nx.block : '[loop-kit] 대기 항목 없음'); return; }
  if (sub === 'abort') {
    setPlanStatus(p, '중단');
...
cmds.fail = (a) => {
  ...
  out(`[loop-kit] ${id} 감사 실패 (시도 ${row.attempt}/${limit}): ${a.reason}`);
  if (row.attempt >= limit) out(`[loop-kit] 재시도 한계 도달, hold ${id} --reason 로 보류하고 사용자 판단 요청`);
  else out(`[loop-kit] 수정 후 start ${id} 로 재착수 (플랜 갱신이 필요하면 먼저 plan revise --ref audit:${id})`);
};
...
cmds.checkpoint = (a) => {
  ...
  const p = readPlan(false);
  out(`[loop-kit] 체크포인트 ${r ? r.version : '(플랜 없음)'} ${r && !r.skipped ? '스냅샷 저장' : '변경 없음'}, 이번 컨텍스트 통과 ${passesThisSession()}개${p ? ', 다음 ' + (nextItem(p) ? nextItem(p).id : '없음') : ''}`);
};
...
cmds.mark = (a) => {
  ...
  out(`[loop-kit] ${id} effect=${a.effect}`);
};
...
cmds.export = () => {
  ...
  out(`[loop-kit] 내보내기 완료 ${path.relative(ROOT, dir)}`);
};

cmds.help = () => out(`loop-kit ${KIT_VERSION}
...
if (!cmds[cmd]) { console.error(`[loop-kit] 알 수 없는 명령 ${cmd}`); cmds.help(); process.exit(1); }
try { cmds[cmd](argv); }
catch (e) { console.error('[loop-kit] 오류: ' + (e && e.message ? e.message : e)); process.exit(1); }
