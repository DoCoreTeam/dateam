...
| 주간보고 | `/weekly-report` | **작성폼이 메인** — 우측 인테이크 패널에서 일일업무·미처리 메모를 체크해 폼에 반영(메모는 반영 시 자동 소진), 전주 계획→이번주 성과 이월. AI 자동초안(일일+캘린더 분석)은 접힘 보조. 지연추적/증빙 포함 |
| 부서업무 | `/dept-tasks` | 부서 단위 업무 관리(조직 스코프 권한) |
| 업무 허브 | `/work` | 일일·주간·부서 업무 통합 뷰 |
