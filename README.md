...
| 이력 | `/work/activity` | **업무 허브 통합 활동이력**(v0.7.287) — 일일·주간·부서·프로젝트의 모든 저장 활동을 한 피드로. 성공/실패/부분 전부, 저장값 스냅샷·실패 원인 표시, 모듈·상태 필터+더보기. 통합 `activity_log`(일일/부서) + `project_activity` + `weekly_report_activity` UNION 정규화 |
