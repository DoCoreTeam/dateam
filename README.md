...
| 이력 | `/work/activity` | **업무 허브 통합 활동이력 + 되살리기**(v0.7.287~289) — 일일·주간·부서·프로젝트의 모든 저장 활동을 한 피드로. **DB 트리거 기반 완전 감사**(v0.7.289 `audit_log`, 마이그146): 앱 어느 경로로 바뀌든 같은 트랜잭션에 자동 기록(빈틈0), `fn_audit` EXCEPTION-guard로 **사용자 저장 절대 비차단** + 2계층 fallback으로 유실0 수렴, before/after 전체행 스냅샷. 각 항목 **되살리기**(본인·항목 단위, 콘텐츠 필드 화이트리스트, 관계/권한 테이블 제외로 권한재획득 차단). daily_logs 소프트삭제 전환(정적가드). 성공=`audit_log`(트리거)·실패=`activity_log`(앱단) |
| 업무 허브 | `/work` | 일일·주간·부서·프로젝트 현황·이력 통합 탭(순서: 일일→주간→부서→프로젝트 현황→이력) |
