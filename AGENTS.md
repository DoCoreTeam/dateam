...
| 카드 | **`className="card"`** | 196 | ~~`NbCard`~~(0·폐기예정), 자작 박스(476) | — 여백은 `--card-pad` 가 준다(§2-3-4). 전면형만 `card-flush`
...
### 2-3-1-b. 읽으러 온 화면은 읽기로 연다 — 같은 값을 쓰는 길은 하나다 (필수 · v0.7.677)
...
- [ ] 버튼·라벨·빈상태·오류 문구를 **`@/lib/terms`에서** 가져옴(화면에 한글 문자열 직접 금지, §0-2)
- [ ] 개수는 `count(entity, n)` · [ ] 새 말은 `lib/terms`에 먼저 추가하고 용어집 등재
