...
이 절은 **보고 첫 줄의 형식**이다. 무엇을 했는지보다 **끝났는지**를 먼저 말한다

왜: 사용자는 보고를 읽고 「이제 손 떼도 되나」를 판단함
판정이 글 속에 섞여 있으면 읽는 쪽은 안전한 쪽으로 가정함, 아직 남았다고
그리고 같은 일을 다시 시킴, 끝난 일을 다시 건드리면 회귀가 생김
(실제 지적 2026-09-20 「다 했으면 다 했다고 정확히 이야기 하라고 정책에 반영하랬자나 다 한건지 모르겠어」)

규칙 넷

1 **첫 줄이 판정이다.** 둘 중 하나를 그대로 씀
  - 전부 됐으면 `완벽히 끝냈습니다`
  - 하나라도 아니면 `아직 안 끝났습니다`
  돌려 말하지 않음, 「마무리했습니다」 「정리됐습니다」 「이상입니다」는 판정이 아님
2 **판정 옆에 숫자를 붙임.** 항목 n/n, 검사 결과(typecheck·lint·test·build), 못 한 것 수
  숫자가 없으면 사용자가 다시 물어야 함
3 **못 한 것은 셋을 적음.** 무엇을·왜·무엇이 있으면 되는지
  「남은 것」이라 쓰지 않음, 그것은 지시 범위를 내가 자른다는 뜻임 (2절 개입 처리와 같은 기준)
4 **부연은 판정 아래로.** 부수 발견·다음 제안·배경이 판정 줄을 흐리지 않게 뒤에 둠

판정 기준

- 플랜의 모든 항목이 통과이고 4절 종합 자가감사가 pass 이고 내 미커밋 변경이 없고 못 한 것이 0개
- 보류가 하나라도 있으면 끝난 것이 아님, 「거의 다」는 미완임
- 틀리게 쓰는 것이 안 쓰는 것보다 나쁨, 한 번 어긋나면 그 문구는 다시는 근거가 되지 못함

- 이 규칙은 새로 만든 것이 아니고 .claude/heavy/CEO.md 의 M-13 을 loop 어법으로 옮긴 것임
  LOOP.md 만 읽는 세션이 그 규칙을 볼 길이 없었음, 버전 규칙과 같은 구멍이었음
- loop CLI 가 final 과 hold 출력에 판정 문장을 그대로 찍음, 보고에 그 줄을 옮김
- 문서와 도구가 갈라지지 않게 apps/web/lib/policy/policy-sync.test.ts 가 셋을 대조함
